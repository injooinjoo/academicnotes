%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 6.431: Probabilistic Systems Analysis - 통합본
% 자동 생성됨
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\title{\textbf{6.431: Probabilistic Systems Analysis}}
\author{통합 강의 노트}
\date{}
...
%=======================================================================
% Lecture 1: 📚 이 교재의 전체 구조 (Roadmap)
%=======================================================================
\chapter{📚 이 교재의 전체 구조 (Roadmap)}
\label{ch:lecture1}
...
\begin{conceptbox}[title={표본 공간은 우리가 노는 '운동장'이다}]
\begin{itemize}
...
\begin{warningbox}[title={Outcome vs Event 구분하기}]
\begin{itemize}
...
\begin{conceptbox}[title={콜모고로프의 3공리}]
\begin{enumerate}
...
\begin{examplebox}[title={공리의 적용: 케이크 자르기}]
\begin{itemize}
...
\begin{storybox}[title={비밀요원 K의 가방 열기 (Counting 시나리오)}]
...
\begin{conceptbox}[title={새로운 정보는 세상을 축소시킨다}]
\begin{itemize}
...
\begin{examplebox}[title={주사위 던지기}]
...
\begin{bayesbox}[title={시간을 거스르는 추론}]
\begin{itemize}
...
\begin{storybox}[title={공장 불량품 추적 시나리오}]
당신은 품질 관리자입니다.
\begin{itemize}
...
\begin{conceptbox}[title={정보의 가치 평가}]
\begin{itemize}
...
\begin{warningbox}[title={독립(Independent) vs 배반(Disjoint)}]
이 둘을 절대 혼동하면 안 됩니다!
\begin{itemize}
...
\begin{conceptbox}[title={Outcome $\to$ Number 변환기}]
\begin{itemize}
...
\begin{examplebox}[title={동전 2개 던지기}]
\begin{itemize}
...
\begin{toolbox}[title={기댓값의 선형성 (Linearity of Expectation)}]
...
\begin{storybox}[title={1. 베르누이 \& 이항 분포 (Binomial)}]
\textbf{"동전 던지기의 반복"}
\begin{itemize}
...
\begin{storybox}[title={2. 기하 분포 (Geometric)}]
...
\begin{storybox}[title={3. 포아송 분포 (Poisson)}]
\textbf{"매우 드문 사건의 카운팅"}
\begin{itemize}
...
\begin{conceptbox}[title={높이가 아니라 '면적'이 확률이다}]
\begin{itemize}
...
\begin{warningbox}[title={점의 확률은 0이다 ($P(X=x)=0$)}]
...
\begin{storybox}[title={1. 균등 분포 (Uniform): "정보 부재의 공평함"}]
\begin{itemize}
...
\begin{storybox}[title={2. 지수 분포 (Exponential): "무기억성(Memoryless)"}]
\begin{itemize}
...
\begin{storybox}[title={3. 정규 분포 (Normal): "세상의 모든 잡음(Noise)"}]
\begin{itemize}
...
\begin{mathstep}[title={모든 정규 분포를 $N(0, 1)$로 만드는 마법}]
...
\begin{examplebox}[title={수능 점수 계산}]
...
\begin{conceptbox}[title={지형도(Surface)의 부피 구하기}]
\begin{itemize}
...
\begin{warningbox}[title={기하학적 함정 (The Geometry Trap)}]
...
\begin{examplebox}[title={삼각형 위에서의 균등 분포}]
...
\begin{storybox}[title={로봇의 위치 추정 (Localization)}]
...
\begin{mathstep}[title={베이지안 추론의 3단계}]
...
\begin{mathstep}[title={CDF 먼저 구하고 미분하라!}]
...
\begin{conceptbox}[title={밀가루 반죽 비유}]
\begin{itemize}
...
\begin{conceptbox}[title={Flip and Drag (뒤집고 밀기)}]
이 적분 식의 기하학적 의미입니다.
\begin{enumerate}
...
\begin{warningbox}[title={Uncorrelated $\neq$ Independent}]
가장 많이 틀리는 함정입니다.
\begin{itemize}
...
\begin{mathstep}[title={LIE 사용법: Divide and Conquer}]
복잡한 $E[X]$를 구할 때:
\begin{enumerate}
...
\begin{storybox}[title={주식 분산 투자의 원리}]
...
\begin{conceptbox}[title={평균의 시소 원리}]
\begin{itemize}
...
\begin{examplebox}[title={연봉 분포 미스터리}]
...
\begin{conceptbox}[title={분산이 알려주는 거리의 제약}]
\begin{itemize}
...
\begin{warningbox}[title={약법칙(Weak) vs 강법칙(Strong)}]
\begin{itemize}
...
\begin{conceptbox}[title={모든 길은 정규 분포로 통한다}]
...
\begin{mathstep}[title={표준화(Standardization) 후 근사}]
...
\begin{storybox}[title={넥슨 서버 개발자의 고민}]
신규 게임 출시를 앞두고 있습니다.
\begin{itemize}
...
\begin{conceptbox}[title={정의: 매 순간의 동전 던지기}]
...
\begin{warningbox}[title={기하 분포와 지수 분포의 관계}]
...
\begin{systembox}[title={1. 병합 (Merging)}]
...
\begin{systembox}[title={2. 분할 (Splitting)}]
...
\begin{storybox}[title={Nexon 게임 기획 및 서버 운영}]
...
\begin{conceptbox}[title={과거 세탁의 원칙}]
\begin{itemize}
...
\begin{warningbox}[title={접근 가능성(Accessibility) 체크}]
...
\begin{conceptbox}[title={Global Balance Equation}]
상태 $j$에 대해:
...
\begin{mathstep}[title={정상 상태($\pi$) 구하는 법}]
\begin{enumerate}
...
\begin{storybox}[title={Nexon 모바일 게임 유저 분석}]
