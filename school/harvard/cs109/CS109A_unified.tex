...
%=======================================================================
% Lecture 1: 과정 개요 (Course Overview)
%=======================================================================
...
\begin{examplebox}[title={지저분한(Messy) 데이터 변환하기}]
...
\begin{examplebox}[title={잘못된 표본 추출 예시}]
\textbf{사례 1: 수업 출석률}
\begin{itemize}
...
\begin{examplebox}[title={어느 것이 더 빠를까? (Mean vs Median)}]
...
\begin{examplebox}[title={Q\&A: 왜 $n$이 아니라 $n-1$로 나누나요?}]
...
\begin{examplebox}[title={사례 연구: 갭마인더(Gapminder)의 5변수 시각화}]
...
\begin{examplebox}[title={잘못된 예: 2020년 미국 대선 지도}]
\begin{itemize}
...
\begin{itemize}
    ...
    \item 데이터 정렬, 집계, 그룹화 기초 (\texttt{sort\\_values}, `groupby`, `agg`)
    \item 정제된 데이터를 파일로 저장하기 (\texttt{to\\_csv})
\end{itemize}
...
\begin{examplebox}[title={데이터의 형태: $X$와 $y$}]
...
\begin{analogybox}[title={아이스크림 비유}]
\begin{itemize}
...
\begin{analogybox}[title={의사의 진단 비유}]
...
\begin{analogybox}[title={시험 공부 비유}]
\begin{itemize}
...
\begin{examplebox}[title={scikit-learn의 \texttt{train\_test\_split}}]
...
\begin{analogybox}[title={NBA 선수 선발 비유}]
...
\begin{defbox}[title={$R^2$ (R-squared)}]
...
\textbf{High Bias (높은 편향 / 정확도 $\downarrow$)} &
  \begin{minipage}{0.45\textwidth}
    ...
    \textbullet~과녁의 중심은 못 맞추지만, 쏜 지점에 계속 일관되게 쏨. \\
\textbullet~(예: 단순 선형 모델)
  \end{minipage} &
  \begin{minipage}{0.45\textwidth}
    \textbf{최악의 모델 (Worst)}
    \textbullet~과녁의 중심도 못 맞추고, 쏠 때마다 아무데나 흩어짐.
  \end{minipage} \\
\addlinespace
\textbf{Low Bias (낮은 편향 / 정확도 $\uparrow$)} &
  \begin{minipage}{0.45\textwidth}
    \textbf{이상적인 모델 (Ideal)}
    \textbullet~과녁의 중심(정답)에 정확하고 일관되게 쏨. \\
\textbullet~\textbf{우리의 목표!}
  \end{minipage} &
  \begin{minipage}{0.45\textwidth}
    \textbf{과대적합 (Overfitting)}
    \textbullet~평균적으로 과녁 중심 근처에 맞지만(훈련 데이터), 쏠 때마다 탄착군이 너무 흩어짐. \\
\textbullet~(예: 고차 다항식 모델)
  \end{minipage} \\
\bottomrule
\end{tabular}
\end{adjustbox}
...
\begin{enumerate}
    ...
    (예: \texttt{Avg\_MSE[$\lambda=0.1$] = (MSE\_k1 + ... + MSE\_k5) / 5})
...
\begin{examplebox}[title={빈도주의 비유: 고정된 보물 찾기}]
...
\begin{examplebox}[title={베이지안 비유: 확률적인 보물 지도}]
...
\begin{examplebox}[title={동전 뒤집기 예제 (이산 확률)}]
    주머니 속에 3개의 동전이 있습니다:
    \begin{itemize}
...
\begin{examplebox}[title={범주형 변수(더미 변수) 해석하기}]
...
\begin{keyconcept}[title={이번 주 학습 목표}]
\begin{itemize}
...
\begin{keyconcept}[title={복잡한 분포를 이해하는 방법: 시뮬레이션}]
...
\begin{analogybox}[title={메트로폴리스-헤이스팅스: 안대 쓴 등산가}]
...
\begin{warningbox}[title={차원의 저주(Curse of Dimensionality)란?}]
...
\begin{analogybox}[title={PCA: 데이터 구름의 '최적의 축' 찾기}]
...
\begin{keyconcept}[title={PCA는 '회전'이다}]
...
\begin{warningbox}[title={PCA는 Y(라벨)를 모른다 (Unsupervised)}]
...
\begin{keyconcept}[title={p-value란 무엇인가?}]
...
\begin{keyconcept}[title={Q: PCA는 지도학습인가요, 비지도학습인가요?}]
...
\begin{keyconcept}[title={Q: 주성분(PC)은 원본 변수와 다른가요?}]
\textbf{A: 완전히 다릅니다.}
...
\begin{keyconcept}[title={Q: PC1이 항상 Y를 가장 잘 예측하나요?}]
\textbf{A: 절대 아닙니다.}
...
\begin{keyconcept}[title={Q: t-검정과 순열 검정 중 무엇을 써야 하나요?}]
...
\begin{warningbox}
  ...
  \texttt{sklearn.linear\_model.LogisticRegression}에서는 $\lambda$ 대신 $C$라는 파라미터를 사용합니다.
...
새로운 데이터가 들어오면, 3개의 분류기를 모두 돌려서 각각의 확률(또는 점수)을 계산한 뒤, 가장 높은 확률(점수)을 보인 클래스로 예측합니다. (`sklearn`의 \texttt{multi\_class='ovr'})
...
\end{itemize}
(`sklearn`의 \texttt{multi\_class='multinomial'})
...
\begin{itemize}
    ...
    \item \textbf{\texttt{hdi\_3%}, \texttt{hdi\_97%} (최고 밀도 구간, Highest Density Interval)}: 사후 분포의 94\% 신뢰 구간입니다 (기본값). $\beta$의 경우 약 28.01에서 40.12 사이입니다.
    \item \textbf{\texttt{mcse\_mean} (MCMC 표준 오차)}: 사후 평균 추정치의 표준 오차입니다. MCMC 샘플이 독립적이지 않기 때문에 단순히 표준편차를 $\sqrt{N}$으로 나눈 것보다 더 정확한 추정치입니다.
    \item \textbf{\texttt{ess\_bulk}, \texttt{ess\_tail} (유효 샘플 크기, Effective Sample Size, ESS)}: MCMC 샘플이 독립적인 관측치와 동일한 정보를 제공하는 정도를 나타냅니다. MCMC 샘플은 서로 상관관계가 있기 때문에, 실제 샘플 수(예: 2000개)보다 ESS가 작습니다. ESS가 클수록 추정치의 신뢰도가 높습니다.
    \item \textbf{\texttt{r\_hat} (R-hat)}: MCMC 체인의 수렴 여부를 진단하는 통계량입니다. 이 값이 \textbf{1에 가까울수록} 체인이 잘 수렴했음을 나타냅니다. 1보다 훨씬 큰 값은 수렴 실패를 의미합니다.
\end{itemize}
\end{infobox}
...
\begin{enumerate}
    \item \textbf{결측치 대체}: 결측된 값을 특정 값(예: 0 또는 해당 변수의 평균/중앙값)으로 대체하여 새로운 변수(예: \texttt{X1\_star})를 만듭니다.
    \item \textbf{지시 변수 생성}: 원래 변수에서 결측치가 있었던 위치를 나타내는 새로운 이진 변수(예: \texttt{X1\_miss})를 생성합니다. 결측치가 있었으면 1, 없었으면 0의 값을 가집니다.
    \item \textbf{모델에 포함}: 분석 시 \texttt{X1\_star}와 \texttt{X1\_miss} 두 변수를 모두 모델에 포함합니다.
\end{enumerate}
\end{infobox}

\begin{examplebox}[title=결측 지시 변수 활용 예시]
\begin{itemize}
    \item \textbf{수치형 변수}: 원래 변수 \texttt{X1}에 결측치가 있다면, \texttt{X1\_star} (결측치를 0으로 대체)와 \texttt{X1\_miss} (결측 여부 지시)를 함께 사용합니다. 이를 통해 모델은 결측치가 0으로 대체된 값의 효과와, 해당 값이 원래 결측이었다는 사실의 효과를 동시에 추정할 수 있습니다.
    \item \textbf{범주형 변수}: 원래 범주형 변수 \texttt{X2} (예: 0 또는 1)에 결측치가 있다면, \texttt{X2\_star} (결측치를 새로운 범주, 예: -1 또는 'missing'으로 대체)와 \texttt{X2\_miss} (결측 여부 지시)를 함께 사용합니다. 이는 사실상 결측치를 하나의 새로운 범주로 취급하는 것과 유사합니다.
\end{itemize}
...
\begin{itemize}
    \item 결측치를 특정 값으로 대체한 변수(\texttt{X\_star})와, 결측 여부를 나타내는 이진 변수(\texttt{X\_miss})를 함께 모델에 포함.
...
%=======================================================================
% Lecture 18: 개요: 왜 로지스틱 회귀가 아닌 결정 트리인가?
%=======================================================================
...
\begin{tcolorbox}{주요 중지 조건 (Hyperparameters)}
    \begin{itemize}
        \item \textbf{\texttt{max\_depth} (최대 깊이)}: 트리가 몇 단계까지 내려갈 수 있는지 제한합니다. (예: \texttt{max\_depth=3}) 가장 흔하고 강력한 규제입니다.
        \item \textbf{\texttt{min\_samples\_leaf} (리프 노드 최소 샘플 수)}: 리프 노드가 되기 위해 필요한 최소한의 샘플 수를 지정합니다. (예: \texttt{min\_samples\_leaf=4}) 만약 어떤 분할이 이 숫자보다 적은 샘플을 가진 리프를 만든다면, 그 분할은 수행되지 않습니다.
        \item \textbf{\texttt{max\_leaf\_nodes} (최대 리프 노드 수)}: 트리 전체의 리프 노드 개수를 제한합니다. (예: \texttt{max\_leaf\_nodes=5})
...
\begin{tcolorbox}[title={결정 트리(Decision Tree) 한눈에 보기}]
    ...
    \begin{itemize}
        ...
        \item \textbf{해결 (Stopping Conditions)}: 트리의 성장을 의도적으로 제한 (사전 가지치기).
            ...
            \end{itemize}
        \item \textbf{튜닝}: 최적의 하이퍼파라미터(예: 최적의 \texttt{max\_depth})는 \textbf{교차 검증(Cross-Validation)}을 통해 찾음.
    \end{itemize}
\end{tcolorbox}


%=======================================================================
% Lecture 19: 개요: 결정 트리 학습의 두 가지 핵심
%=======================================================================
...
\begin{boxDefinition}[title={회귀 트리 (Regression Tree)란?}]
...
\begin{boxDefinition}[title={회귀 트리의 분할 기준: MSE 최소화}]
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \texttt{max\_depth}: 트리의 최대 깊이
        \item \texttt{min\_samples\_leaf}: 단말 노드가 가져야 할 최소 샘플 수
...
\begin{boxDefinition}[title={원-핫 인코딩 (OHE)}]
...
\begin{boxWarning}
    ...
    모델에 데이터를 넣기 전에, 사용자가 직접 Pandas의 \texttt{get\_dummies}나 `sklearn.preprocessing.OneHotEncoder`를 사용하여 OHE 전처리를 \textbf{반드시} 수행해야 합니다.
...
\begin{enumerate}
    ...
    트리가 성장하는 \textit{중에} 멈추는 방식입니다. (\texttt{max\_depth}, \texttt{min\_samples\_leaf} 등)
    \begin{itemize}
...
\begin{boxDefinition}[title={비용 복잡도 (Cost Complexity) 공식}]
...
\begin{boxExample}
    \textbf{Q5: \texttt{max\_depth=1}로 설정된 결정 트리의 결정 경계(Decision Boundary)는 선형(Linear)인가요, 비선형(Non-linear)인가요?}
    
    \textbf{A: 선형(Linear)입니다.}
    \texttt{max\_depth=1}은 트리가 \textbf{단 하나의 분할}만 수행한다는 의미입니다. (예: "x > 6.5")
...
%=======================================================================
% Lecture 21: 개요: 배깅(Bagging) 기법 및 OOB 오류
%=======================================================================
...
\begin{examplebox}[title={현실 비유: MRI 진단}]
...
\begin{examplebox}[title={트리의 상관관계 예시}]
...
\begin{summarybox}[title={배깅(Bagging) 및 OOB 핵심 정리}]
\begin{itemize}
...
\begin{tipbox}[title={신경망 vs. 부스팅/랜덤 포레스트}] % 마스터 템플릿의 tipbox 사용
...
\begin{tipbox}[title={머신러닝에서의 '추론(Inference)' 용어}] % 마스터 템플릿의 tipbox 사용
...
\begin{tipbox}[title={팁: 스텀프는 어떻게 가중치를 반영하나요?}]
...
\begin{warningbox}[title={과제/프로젝트 주의사항}]
...
%=======================================================================
% Lecture 25: 개요: 왜 더 복잡한 앙상블이 필요한가?
%=======================================================================
...
\begin{summarybox}[title={핵심 목표 및 요약}]
\begin{itemize}
...
\begin{analogybox}[title={어벤져스 팀 구성하기}]
\begin{itemize}
...
\begin{warningbox}[title={Blending의 단점: 데이터 효율성}]
...
\begin{analogybox}[title={Stacking 과정 비유}]
팀원들이 돌아가며 모의고사를 봅니다.
\begin{itemize}
...
\begin{analogybox}[title={병원 진료 시스템}]
