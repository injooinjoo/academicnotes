...
\hypersetup{
    pdftitle={CSCI E-89: Introduction to Artificial Intelligence - Module 03},
    pdfauthor={강의 노트},
...
강의에서는 \texttt{SimpleRNN(2, input_shape=(None, 4))}와 같은 Keras 코드를 예시로 들었습니다.
\begin{itemize}
    \item \texttt{SimpleRNN(2)}: 이 순환 레이어에는 2개의 뉴런(또는 은닉 상태)이 있습니다. 이는 출력 \texttt{y} 벡터가 2차원임을 의미합니다.
    \item \texttt{input_shape=(None, 4)}: 입력 데이터의 형태를 나타냅니다.
        \begin{itemize}
...
\texttt{SimpleRNN(2, input_shape=(None, 4))}의 경우:
\begin{itemize}
    \item 입력 특성 수 = 4
    \item 은닉 뉴런 수 = 2
\end{itemize}
...
RNN은 시퀀스 데이터를 처리하는 방식에 따라 다양한 입출력 구조를 가질 수 있습니다. Keras에서는 \texttt{return_sequences} 옵션을 통해 출력을 제어합니다.

\begin{itemize}
    \item \textbf{다대다 (Many-to-Many):} 입력 시퀀스에 대해 출력 시퀀스를 생성합니다.
        \begin{itemize}
            \item 예시: 기계 번역 (입력 문장 -> 출력 문장).
            \item \texttt{return_sequences=True} (기본값)로 설정하면 각 시간 스텝의 출력을 모두 반환합니다.
        \end{itemize}
    \item \textbf{다대일 (Many-to-One):} 입력 시퀀스에 대해 단일 출력을 생성합니다.
        \begin{itemize}
            \item 예시: 감성 분석 (입력 문장 -> 긍정/부정).
            \item \texttt{return_sequences=False}로 설정하면 마지막 시간 스텝의 출력만 반환합니다.
        \end{itemize}
...
RNN 레이어는 기본적으로 각 시간 스텝마다 출력을 생성합니다. \texttt{return_sequences=True}는 이 모든 출력을 다음 레이어로 전달하는 반면, \texttt{return_sequences=False}는 마지막 시간 스텝의 출력만 전달합니다. 이는 모델의 목적에 따라 적절히 선택해야 합니다.
\end{tcolorbox}
...
\begin{itemize}
    \item \textbf{데이터셋:} \texttt{sklearn.datasets.fetch_20newsgroups}를 사용하여 20개의 뉴스그룹 중 'rec.sport.hockey'와 'misc.forsale' 두 가지 카테고리만 선택합니다.
...
\begin{itemize}
    ...
    \item \textbf{작동 원리:}
        \begin{itemize}
            ...
            \item \texttt{Embedding(input_dim, output_dim)}:
                \begin{itemize}
                    \item \texttt{input_dim}: 어휘 사전의 크기 (총 단어 수 + OOV 토큰).
                    \item \texttt{output_dim}: 임베딩 벡터의 차원 (예: 128).
                \end{itemize}
        \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item \textbf{Embedding Layer:} \texttt{max_features + 1} (어휘 사전 크기 + OOV) 차원의 입력을 \texttt{embedding_dim} (예: 128) 차원의 밀집 벡터로 변환합니다. \texttt{input_length}는 각 시퀀스의 최대 길이입니다.
    \item \textbf{Dropout Layer:} 임베딩 레이어의 출력에 적용됩니다.
    \item \textbf{LSTM Layer:} 128개의 은닉 상태를 가지는 LSTM 레이어입니다. \texttt{return_sequences}가 지정되지 않았으므로 기본값인 \texttt{False}가 적용되어 마지막 시간 스텝의 출력만 다음 레이어로 전달됩니다.
...
\begin{itemize}
    ...
    \item \textbf{OOV (Out-Of-Vocabulary) 토큰:} \texttt{max_features}에 포함되지 않는 단어들을 처리하기 위한 특별한 토큰입니다. 일반적으로 인덱스 0을 OOV 토큰에 할당합니다.
    \item \textbf{단어 빈도 계산 및 인덱싱:}
        \begin{enumerate}
            \item 훈련 텍스트를 토큰화합니다.
            \item 각 토큰의 빈도를 계산합니다.
            \item 빈도 순으로 정렬하여 상위 \texttt{max_features}개의 단어만 선택합니다.
...
\begin{itemize}
    \item \textbf{훈련:} \texttt{model.fit()} 함수를 사용하여 훈련 데이터와 레이블로 모델을 훈련합니다. \texttt{epochs} (훈련 반복 횟수)와 \texttt{batch_size} (미니 배치 크기)를 지정합니다.
...
\begin{itemize}
    ...
    \item \textbf{시퀀스 패딩:} \texttt{pad_sequences}를 사용하여 시퀀스 길이를 통일했는가?
...
\textbf{A1:} \texttt{SimpleRNN(은닉 뉴런 수, input_shape=(None, 입력 특성 수))}의 경우, 파라미터 수는 \texttt{(입력 특성 수 * 은닉 뉴런 수) + (은닉 뉴런 수 * 은닉 뉴런 수) + 은닉 뉴런 수}로 계산됩니다. 이는 입력 가중치, 순환 가중치, 바이어스의 합입니다.
\end{tcolorbox}
...
\begin{itemize}
    ...
    \item \textbf{입출력 유형:} 다대다, 다대일, 일대다 (Keras \texttt{return_sequences} 옵션).
\end{itemize}
