...
\begin{lstlisting}[language=Python, caption={TfidfVectorizer 기본 사용 예제}, label={code:tfidf_basic}]
...
\begin{lstlisting}[language=Python, caption={NLTK Stemming을 적용한 TF-IDF}, label={code:tfidf_stemming}]
import nltk
...
\subsection{모델 학습}
\begin{lstlisting}[language=Python, caption={Gensim으로 Word2Vec 모델 학습하기}, label={code:word2vec_train}]
...
\begin{lstlisting}[language=Python, caption={가장 유사한 단어 찾기}, label={code:word2vec_similar}]
# 'cat'과 가장 유사한 단어 찾기
...
\begin{lstlisting}[language=Python, caption={'king-man+woman' 유추 문제 풀이}, label={code:word2vec_analogy}]
