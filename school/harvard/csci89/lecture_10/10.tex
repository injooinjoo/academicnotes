...
\begin{examplebox}
    ...
    \begin{lstlisting}[language=Python, caption={규칙 기반 NER을 위한 정규표현식 예시 (날짜, 이메일 등)}, label={lst:regex_ner1}]
    import re
...
\begin{examplebox}
    ...
    \begin{lstlisting}[language=Python, caption={호칭(Title) 기반 정규표현식 예시}, label={lst:regex_ner2}]
...
\begin{lstlisting}[language=Python, caption={NLTK를 사용한 문장 토큰화 및 POS Tagging}, label={lst:nltk_pos}]
import nltk
...
\begin{lstlisting}[language=Python, caption={spaCy를 사용한 NER 수행}, label={lst:spacy_ner}]
import spacy
...
\begin{examplebox}
    ...
    \begin{lstlisting}[language=Python, caption={spaCy NER 모델 미세조정(Fine-Tuning) 시도}, label={lst:spacy_finetune}]
    import spacy
    import random
