...
\hypersetup{
    pdftitle={CSCI E-89: Introduction to Artificial Intelligence - Module 02},
    pdfauthor={강의 노트},
...
def generate_garch_like_series(n_steps=1000, alpha=0.1, beta=0.8, gamma=0.05, interaction=0.1):
    ...
    \begin{itemize}
        \item \textbf{예시:} \texttt{time_steps = 14}라면, \texttt{[X_t, X_{t+1}, ..., X_{t+13}]}이 입력 시퀀스가 되고, \texttt{X_{t+14}}가 예측할 출력 값이 됩니다.
    \end{itemize}
...
model.summary()
    ...
    \begin{tipbox}
    \textbf{\texttt{return_sequences} 옵션}
    \begin{itemize}
        \item \textbf{\texttt{return_sequences=True}:} 해당 RNN 층의 모든 시간 단계에 대한 은닉 상태 시퀀스를 출력합니다. 이는 다음 RNN 층이 전체 시퀀스 정보를 입력으로 받아야 할 때 사용됩니다.
        \item \textbf{\texttt{return_sequences=False} (기본값):} 해당 RNN 층의 마지막 시간 단계의 은닉 상태만 출력합니다. 이는 RNN 층 다음에 완전 연결 층(Dense Layer)을 연결하여 최종 예측 값을 만들 때 주로 사용됩니다.
    \end{itemize}
    \end{tipbox}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{훈련 데이터(\texttt{x_train}, \texttt{y_train}):} 모델 학습에 사용될 데이터.
        \item \textbf{에포크(\texttt{epochs}):} 전체 훈련 데이터를 몇 번 반복하여 학습할지 지정.
        \item \textbf{배치 크기(\texttt{batch_size}):} 한 번의 가중치 업데이트에 사용될 샘플 수.
        \item \textbf{검증 데이터(\texttt{validation_data}):} 훈련 중 모델의 일반화 성능을 평가하기 위한 데이터.
        \item \textbf{콜백(\texttt{callbacks}):} EarlyStopping, ModelCheckpoint 등을 사용하여 학습 과정 제어.
    \end{itemize}
    \item \textbf{모델 평가 (\texttt{model.evaluate()}):}
    \begin{itemize}
        \item \textbf{테스트 데이터(\texttt{x_test}, \texttt{y_test}):} 모델의 최종 성능을 평가하기 위한 데이터.
...
\begin{itemize}
    \item \textbf{증상:} \texttt{ValueError: Input 0 of layer "simple_rnn" is incompatible with the layer: expected ndim=3, found ndim=2. Full shape received: (None, 14)}
...
해결 방법 & \texttt{data.reshape(-1, time_steps)} & \texttt{data.reshape(-1, time_steps, 1)} \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}
