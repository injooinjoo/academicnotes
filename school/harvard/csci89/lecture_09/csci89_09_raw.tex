...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{CSCI89 Lecture 9: 고전적 머신러닝 기법}
\fancyhead[R]{\today}
\fancyfoot[C]{\thepage}
...
% PDF 메타데이터 설정
\hypersetup{
  pdftitle={CSCI89 Lecture 9: 고전적 머신러닝 기법},
...
\begin{infobox}{NLP에 고전적 모델 적용하기}
\begin{itemize}
...
\begin{infobox}{TF-IDF Vectorizer의 역할}
\begin{itemize}
...
\begin{summarybox}{K-fold 교차 검증의 필요성}
\begin{itemize}
...
\begin{infobox}{Grid Search CV의 작동 원리}
\begin{itemize}
...
\begin{infobox}{혼동 행렬 (Confusion Matrix)}
...
\begin{warningbox}{불균형 데이터셋과 평가 지표}
...
\begin{infobox}{랜덤 포레스트 (기본 설정) 결과 해석}
\begin{itemize}
    \item \textbf{정확도}: 약 0.9802
    \item \textbf{민감도 (Recall for Tech)}: 약 0.8903
    \item \textbf{특이도 (Specificity for Not Tech)}: 약 0.9978
...
\begin{infobox}{랜덤 포레스트 (최적) 결과 해석}
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{LaTeX 형식}:
    \begin{itemize}
        \item `\documentclass`, `\usepackage`, `\geometry`, `\onehalfspacing`, `\parskip`, `\parindent`, `\arraystretch`가 올바르게 설정되었는가?
        \item `fancyhdr`로 헤더/푸터가 설정되었는가?
        \item `tcolorbox`로 요약/경고/예시 박스가 사용되었는가?
        \item `listings`로 코드 블록이 올바르게 포맷되었는가? (캡션, 라벨, 행 번호, `breaklines=true`)
        \item 모든 표/그림/코드에 캡션과 라벨이 있는가?
        \item 모든 표는 `\textwidth`에 맞게 조정되었는가?
        \item 각 주요 섹션 시작 전 `\newpage`가 사용되었는가?
        \item 출처, 파일명, 페이지 언급이 없는가?
        \item 특수문자 이스케이프가 올바르게 적용되었는가?
        \item 마크다운 굵게, 백틱, `\fbox` 내 `itemize` 등 금지 사항을 준수했는가?
    \end{itemize}
    \item \textbf{초심자 친화성}:
    \begin{itemize}
...
\textbf{A1:} TF-IDF Vectorizer는 문서 빈도(IDF) 정보를 계산할 때 훈련 데이터만을 사용해야 합니다. K-fold 교차 검증 시, 각 폴드마다 훈련 세트가 달라지므로, Vectorizer도 매번 해당 훈련 세트에 맞춰 다시 학습(fit)되어야 합니다. 파이프라인에 Vectorizer를 포함하면, K-fold가 자동으로 각 훈련 폴드에 대해 Vectorizer를 `fit`하고, 해당 테스트 폴드에 `transform`을 적용하여 데이터 누수(Data Leakage)를 방지하고 일관된 전처리 과정을 보장합니다.
