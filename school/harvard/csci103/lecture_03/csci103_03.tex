...
\hypersetup{
    pdftitle={CSCI E-103: Introduction to the Intellectual Enterprises of CS - Module 03},
    pdfauthor={강의 노트},
...
\begin{itemize}
    ...
    \item 이번 실습에서는 \texttt{/databricks-datasets/log_files/} 경로의 Apache 로그 파일을 사용합니다.
\end{itemize}
...
\begin{itemize}
    \item \texttt{dbutils.fs.head(log_files_path + "part-00000")} 명령으로 로그 파일의 첫 몇 줄을 확인합니다.
...
\begin{itemize}
    \item \texttt{spark.read.text(log_files_path)}를 사용하여 로그 파일을 텍스트 데이터프레임으로 읽어옵니다. 이 데이터프레임은 각 로그 라인을 하나의 문자열 컬럼으로 가집니다.
    \item \texttt{display(raw_log_files_df)} 명령으로 데이터프레임 내용을 확인합니다. \texttt{display}는 Spark 작업을 트리거하는 액션(Action)입니다.
\end{itemize}
...
\begin{itemize}
    \item \texttt{pyspark.sql.functions.regexp_extract} 함수를 사용하여 로그 문자열에서 IP 주소, 사용자 ID, 타임스탬프, 요청 메서드, 엔드포인트, 응답 코드, 콘텐츠 크기 등을 추출합니다.
...
\begin{itemize}
    \item \texttt{parsed_df.count()}를 통해 파싱된 레코드 수를 확인합니다. 원본과 동일한 100,000개 레코드가 파싱되었는지 확인합니다.
    \item \texttt{parsed_df.createOrReplaceTempView("log_data")} 명령으로 파싱된 데이터프레임을 SQL 뷰로 등록합니다. 이는 현재 Spark 세션 내에서만 유효합니다.
\end{itemize}
...
\begin{itemize}
    \item 로그에서 추출한 \texttt{timestamp_str}은 \texttt{[10/Oct/2023:10:00:00 +0000]}와 같은 특정 형식의 문자열입니다.
    \item \texttt{CAST(timestamp_str AS TIMESTAMP)}와 같이 일반적인 형변환으로는 올바르게 파싱되지 않고 \texttt{NULL}이 반환됩니다. 이는 Spark가 해당 문자열 형식을 자동으로 인식하지 못하기 때문입니다.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{활용:} 데이터프레임 API의 \texttt{select} 함수 내에서 UDF를 호출하여 \texttt{timestamp_str}을 Unix 타임스탬프(Epoch Time)로 변환합니다.
\end{itemize}
...
\end{lstlisting}
\textit{위 코드에서는 \texttt{FROM_UNIXTIME} 함수를 사용하여 Unix 타임스탬프를 사람이 읽기 쉬운 \texttt{yyyy-MM-dd HH:mm:ss} 형식으로 변환했습니다.}

\subsection{데이터 정제 및 변환}

\subsubsection{데이터프레임 컬럼 변환 및 이름 변경}
\begin{itemize}
    \item \texttt{content_size}와 \texttt{response_code} 컬럼을 정수형(INT)으로 변환합니다.
    \item \texttt{timestamp_str}을 UDF를 통해 \texttt{event_time}으로 변환하고, 필요한 경우 \texttt{FROM_UNIXTIME}과 같은 Spark SQL 함수를 사용하여 날짜/시간 형식을 지정합니다.
...
\begin{itemize}
    \item \texttt{clean_logs_df} (로그 데이터)와 \texttt{status_codes_df} (상태 코드 참조 데이터)를 조인합니다.
    \item 조인 조건은 \texttt{response_code} 컬럼입니다. 이때, 두 데이터프레임의 조인 키 컬럼의 데이터 타입이 일치해야 합니다. \texttt{clean_logs_df}의 \texttt{response_code}는 INT형이고, \texttt{status_codes_df}의 \texttt{code}는 STRING형이므로, 조인 전에 INT형 \texttt{response_code}를 STRING형으로 형변환해야 합니다.
\end{itemize}
...
\begin{itemize}
    \item 조인된 \texttt{network_df}에서 특정 조건을 만족하는 레코드만 필터링합니다.
    \item \textbf{예시:} 응답 코드가 200(성공)이고, \texttt{content_size}가 100보다 큰 요청만 필터링합니다. 이는 애플리케이션에서 성공했지만 처리량이 많은 요청을 분석하는 데 유용할 수 있습니다.
\end{itemize}
...
\begin{itemize}
    \item \texttt{filtered_network_df.write.format("delta").mode("overwrite").saveAsTable("network_filter")} 명령을 사용하여 필터링된 데이터를 \texttt{network_filter}라는 Delta 테이블에 저장합니다.
...
\begin{itemize}
    \item \texttt{SELECT * FROM network_filter} 쿼리를 실행하여 저장된 데이터를 확인합니다.
    \item \texttt{SELECT COUNT(*) FROM network_filter}를 통해 저장된 레코드 수를 확인합니다. 이는 필터링된 데이터프레임의 레코드 수와 일치해야 합니다.
    \item Databricks 카탈로그 탐색기에서 \texttt{lab02} 스키마 아래에 \texttt{network_filter} 테이블이 생성되었는지 확인할 수 있습니다.
\end{itemize}
...
\begin{itemize}
    \item \texttt{filtered_network_df.explain()} 명령을 실행하여 Spark의 실행 계획을 확인합니다.
    \item \textbf{실행 계획 해석:}
    \begin{itemize}
...
\begin{enumerate}
    ...
    \item \texttt{regexp_extract}로 IP, 타임스탬프, 엔드포인트 등 파싱.
