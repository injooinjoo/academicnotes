...
\lstset{
  ...
  xleftmargin=0.75em, % 왼쪽 여백
  xrightmargin=0.75em, % 오른쪽 여백
  showstringspaces=false, % 문자열 내 공백 표시 안 함
  commentstyle=\color{gray}\ttfamily, % 주석 스타일
  keywordstyle=\color{blue}\ttfamily, % 키워드 스타일
  stringstyle=\color{red}\ttfamily % 문자열 스타일
}

% PDF 메타데이터 설정
\hypersetup{
  pdftitle={CSCI103 Lecture 11: LLM 및 에이전트 개발 및 배포},
...
\fancyhead[L]{CSCI103 Lecture 11: LLM 및 에이전트 개발 및 배포} % 왼쪽 헤더
...
\newpage
\section*{개요}
이 문서는 CSCI103 Lecture 11 강의 자료를 바탕으로 대규모 언어 모델(LLM)과 에이전트의 개발 및 배포에 대한 포괄적인 내용을 다룹니다. AI의 진화 과정부터 LLM의 핵심 개념, 활용 사례, 그리고 실제 구현 시 발생할 수 있는 도전 과제와 윤리적 고려 사항까지 상세히 설명합니다. 특히, Databricks 플랫폼을 활용한 LLM 기능 데모를 통해 이론적 지식을 실제 적용하는 방법을 제시하며, 초심자도 쉽게 이해할 수 있도록 풍부한 설명과 예시를 제공합니다.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{ai_evolution_venn_diagram.png} % 이미지 파일이 없으므로 가상의 파일명 사용
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{databricks_platform_overview.png} % 이미지 파일이 없으므로 가상의 파일명 사용
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{AI Functions:} SQL 문에서 직접 Gen AI 기능을 호출할 수 있습니다 (예: `summarize`, `sentiment_analysis`).
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{ai_maturity_curve.png} % 이미지 파일이 없으므로 가상의 파일명 사용
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{예시:} `SELECT AI_SIMILARITY('text1', 'text2'), AI_SUMMARIZE('long text')`와 같이 SQL 쿼리에서 감성 분석, 요약, 정보 추출 등을 수행합니다.
    \end{itemize}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{rag_architecture.png} % 이미지 파일이 없으므로 가상의 파일명 사용
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{llm_ops_blueprint.png} % 이미지 파일이 없으므로 가상의 파일명 사용
    \caption{LLM Ops 청사진}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item `get_a_quote`: 특정 고객에 대한 보장, 예상 보험료, 상품 옵션, 건강 등급 등 모든 견적 세부 정보를 반환합니다. (매개변수: `client_name` (문자열))
            \item `get_quote_status`: 견적 상태를 반환합니다.
            \item `get_risk_score`: 위험 점수를 반환합니다.
        \end{itemize}
        \item \textbf{특징:}
        \begin{itemize}
...
\begin{tcolorbox}[summarybox={CSCI103 Lecture 11: LLM 및 에이전트 핵심 요약}]
    ...
    \begin{itemize}
        ...
        \item \textbf{Agent Bricks:} 에이전트 자동 생성 및 오케스트레이션.
