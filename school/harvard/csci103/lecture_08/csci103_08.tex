...
\hypersetup{
    pdftitle={CSCI E-103: Introduction to the Intellectual Enterprises of CS - Module 08},
    pdfauthor={강의 노트},
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{AutoLog:} MLflow는 \texttt{autolog()} 기능을 제공하여, 주요 ML 라이브러리(Scikit-learn, TensorFlow 등)의 매개변수와 지표를 자동으로 기록할 수 있습니다. 추가적인 정보는 \texttt{mlflow.log_param()}, \texttt{mlflow.log_metric()}, \texttt{mlflow.log_artifact()} 등을 사용하여 수동으로 기록할 수 있습니다.
...
\begin{itemize>
    ...
    \item \textbf{모델 URI} (Model URI): MLflow Tracking 서버나 Model Registry에 저장된 모델을 고유하게 식별하는 경로입니다. \texttt{runs:/<run_id>/<artifact_path>} 또는 \texttt{models:/<model_name>/<version_or_alias>} 형식으로 사용됩니다.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{방법:} \texttt{mlflow.start_run()}으로 실험 실행을 시작하고, \texttt{mlflow.log_param()}, \texttt{mlflow.log_metric()}, \texttt{mlflow.log_artifact()} 등으로 정보를 기록합니다. \texttt{mlflow.autolog()}를 사용하면 주요 라이브러리의 정보를 자동으로 기록할 수 있습니다.
    ...
    \begin{itemize}
        ...
        \item \textbf{방법:} \texttt{mlflow.<flavor>.log_model()} 함수를 사용하여 모델을 Tracking 서버에 로깅한 후, \texttt{mlflow.register_model()} 또는 \texttt{mlflow.<flavor>.log_model(registered_model_name=...)}을 통해 Model Registry에 등록합니다.
        \item \textbf{결과:} 모델은 고유한 이름과 버전 번호를 가지며, \texttt{Staging}, \texttt{Production} 등의 스테이지로 전환될 수 있습니다. \texttt{mlflow.set_registered_model_alias()}를 사용하여 'best', 'latest'와 같은 별칭을 부여할 수 있습니다.
    \end{itemize}
    \item \textbf{모델 배포 및 예측 (MLflow Models):}
    \begin{itemize}
        \item \textbf{목표:} 등록된 모델을 로드하여 새로운 데이터에 대한 예측을 수행하고, 다양한 환경에 배포합니다.
        \item \textbf{방법:} \texttt{mlflow.pyfunc.load_model()} 또는 \texttt{mlflow.<flavor>.load_model()}을 사용하여 모델을 로드합니다. 모델 URI(\texttt{models:/<model_name>/<version_or_alias>})를 통해 Registry에서 모델을 참조합니다.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{mlflow.start_run(nested=True)}를 사용하여 각 튜닝 시도를 별도의 하위 실행으로 기록하면, 각 시도의 매개변수와 지표를 쉽게 비교하고 관리할 수 있습니다.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \texttt{mlflow.start_run()}을 사용하여 실험 실행을 시작하고 매개변수, 지표, 아티팩트를 로깅할 수 있는가?
        \item \texttt{mlflow.autolog()}의 기능을 이해하고 활용할 수 있는가?
        \item MLflow Model Registry에 모델을 등록하고 버전 및 스테이지를 관리할 수 있는가?
        \item \texttt{models:/<model_name>/<version_or_alias>} 형식의 모델 URI를 사용하여 모델을 로드할 수 있는가?
        \item Spark UDF를 사용하여 등록된 모델로 대규모 배치 추론을 수행할 수 있는가?
    \end{itemize}
    \item \textbf{실습 준비:}
    \begin{itemize}
        \item 제공된 코드 예시를 Databricks 환경에서 실행할 수 있는가?
        \item Unity Catalog 이름(\texttt{CSCI103_catalog})을 올바르게 설정했는가?
        \item Hyperopt 실습 시 \texttt{SparkTrials} 대신 \texttt{Trials()}를 사용하도록 수정했는가?
        \item 하이퍼파라미터 튜닝의 \texttt{n_trials} 또는 \texttt{max_evals} 값을 적절히 조절하여 실행 시간을 단축했는가?
    \end{itemize}
    \item \textbf{문제 해결:}
    \begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Q: MLflow에서 \texttt{runs:/<run_id>/<artifact_path>}와 \texttt{models:/<model_name>/<version_or_alias>}는 언제 사용하나요?}
    \textbf{A:} \texttt{runs:/<run_id>/<artifact_path>}는 \textbf{MLflow Tracking 서버}에 저장된 특정 실험 실행(run)의 아티팩트(예: 모델 파일)를 참조할 때 사용합니다. 이는 주로 실험 단계에서 특정 실행의 모델을 로드하여 테스트할 때 유용합니다. 반면, \texttt{models:/<model_name>/<version_or_alias>}는 \textbf{MLflow Model Registry}에 등록된 모델을 참조할 때 사용합니다. 이는 모델이 검증을 거쳐 '공식적인' 자산으로 관리될 때 사용하며, 버전 번호나 'best', 'latest'와 같은 별칭을 통해 모델을 참조할 수 있어 배포 및 운영 단계에서 더 편리하고 안정적입니다.
\end{itemize}
\end{tcolorbox}
