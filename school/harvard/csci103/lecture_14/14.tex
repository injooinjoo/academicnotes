...
\begin{enumerate}
    ...
    \item \textbf{Broadcast Variables}:
        \begin{itemize}
            \item **Immutable (불변)**: 생성 후 변경할 수 없습니다.
            \item **Local to Workers**: 클러스터 간 공유되는 것이 아니라, 각 워커 노드에 복사본이 저장됩니다.
        \end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{구조}: 각 트랜잭션은 **개별 JSON 파일**로 기록됩니다. (하나의 JSON이 아님). 10번째 트랜잭션마다 Checkpoint(Parquet) 파일이 생성됩니다.
