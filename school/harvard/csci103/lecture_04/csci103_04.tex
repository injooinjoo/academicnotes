...
\hypersetup{
    pdftitle={CSCI E-103: Introduction to the Intellectual Enterprises of CS - Module 04},
    pdfauthor={강의 노트},
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{예시:} \texttt{OPTIMIZE table_name;} 또는 \texttt{OPTIMIZE table_name WHERE date >= '2023-01-01';}
    \end{itemize}
    \item \textbf{Z-오더링 (Z-Ordering):}
    \begin{itemize}
        \item \textbf{한 줄 요약:} 여러 컬럼에 걸쳐 관련 데이터를 물리적으로 가깝게 저장하여 다차원 쿼리 성능을 최적화하는 기법입니다.
        \item \textbf{필요성:} 특정 쿼리가 여러 컬럼(예: \texttt{from_ip}, \texttt{to_ip}, \texttt{from_port}, \texttt{to_port})을 동시에 필터링할 때, 이 컬럼들의 데이터가 디스크에 흩어져 있으면 많은 I/O가 발생합니다.
        ...
        \item \textbf{예시:} \texttt{OPTIMIZE table_name ZORDER BY (from_ip, to_ip, from_port, to_port);}
    \end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{예시:} \texttt{VACUUM my_delta_table RETAIN 168 HOURS;} (7일 = 168시간)
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{기술적 설명:} \texttt{@pandas_udf} 데코레이터를 사용하여 정의하며, Spark와 Python 간의 데이터 전송을 배치(batch) 단위로 처리하여 직렬화/역직렬화 횟수를 줄여 성능을 향상시킵니다.
    \end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{기술적 설명:} Delta Lake의 변경 데이터 피드(Change Data Feed) 기능을 활성화하면, 테이블의 버전 간 행 수준 변경 사항을 기록하는 별도의 폴더(\texttt{_change_data})가 생성됩니다. 이 피드는 각 행이 삽입(\texttt{INSERT}), 삭제(\texttt{DELETE}), 업데이트(\texttt{UPDATE})되었는지에 대한 메타데이터를 포함합니다.
    \item \textbf{예시:}
...
\end{lstlisting}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item A2 Z2 (A2 B2가 A2 Z2로 변경됨) -> \texttt{pre_image} (B2), \texttt{post_image} (Z2), \texttt{operation} (UPDATE)
            \item A3 (A3 B3가 삭제됨) -> \texttt{pre_image} (B3), \texttt{post_image} (NULL), \texttt{operation} (DELETE)
            \item A4 B4 (A4 B4가 새로 삽입됨) -> \texttt{pre_image} (NULL), \texttt{post_image} (B4), \texttt{operation} (INSERT)
        \end{itemize}
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{일반적인 구현:} \texttt{start_date}, \texttt{end_date}, \texttt{is_current}와 같은 추가 컬럼을 사용하여 각 레코드의 유효 기간과 현재 상태를 나타냅니다.
        \item \textbf{예시:} 고객의 주소가 변경되면, 기존 주소 레코드의 \texttt{end_date}를 업데이트하고 \texttt{is_current} 플래그를 \texttt{false}로 설정합니다. 그리고 새로운 주소로 \texttt{start_date}와 \texttt{is_current}가 \texttt{true}인 새 레코드를 삽입합니다.
    \end{itemize}
...
\begin{itemize}
    \item \textbf{초기 테이블 (추가 컬럼: \texttt{start_date}, \texttt{end_date}, \texttt{is_current}):}
        \begin{itemize}
    ...
    \item \textbf{최종 테이블 (SCD Type 2):}
        ...
        \end{itemize}
    \item 고객 2의 데이터도 \texttt{is_current}가 \texttt{FALSE}로 변경되어 이력이 유지됩니다.
\end{itemize}
\end{examplebox}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item \texttt{schemaEvolutionMode("rescue")}: 스키마와 일치하지 않는 데이터를 \texttt{_rescued_data}라는 특수 컬럼에 저장합니다. 이 컬럼이 비어있지 않으면 잘못된 데이터가 있음을 나타내며, 이를 분석하여 데이터 품질 문제를 해결할 수 있습니다.
    ...
    \begin{itemize}
        ...
        \item \textbf{예시:} \texttt{option("cloudFiles.schemaHints", "signal_strength FLOAT, timestamp LONG, metadata MAP<STRING, STRING>")}
    \end{itemize}
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \texttt{option("cloudFiles.schemaLocation", "/path/to/schema_location")}으로 스키마 저장 위치를 지정합니다.
        ...
        \item 이 단계에서 Autoloader는 초기 데이터로부터 스키마를 추론하고, \texttt{_rescued_data} 컬럼을 기본적으로 추가합니다.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{DESCRIBE FORMATTED table_name} 명령으로 업데이트된 스키마를 확인하면 \texttt{metadata} 컬럼이 추가된 것을 볼 수 있습니다.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item 파싱되지 못한 데이터는 \texttt{_rescued_data} 컬럼에 JSON 형태로 저장됩니다. 이를 통해 어떤 파일의 어떤 데이터가 문제였는지 식별할 수 있습니다.
    \end{itemize}
    \item \textbf{스키마 힌트 적용 및 데이터 복구:}
    \begin{itemize}
        \item 스트림을 재설정하고, \texttt{cloudFiles.schemaHints} 옵션을 사용하여 \texttt{signal_strength}를 \texttt{FLOAT}, \texttt{timestamp}를 \texttt{LONG}, \texttt{metadata}를 \texttt{MAP}으로 명시적으로 지정합니다.
        \item 다시 잘못된 타임스탬프 데이터를 주입하면, \texttt{_rescued_data} 컬럼에 저장된 잘못된 타임스탬프 값을 \texttt{CAST}하여 올바른 형식으로 복구하는 로직을 적용할 수 있습니다.
        \item \texttt{SELECT timestamp, CAST(get_json_object(_rescued_data, '$.timestamp') AS BIGINT) AS repaired_timestamp FROM bronze_table WHERE _rescued_data IS NOT NULL}과 같은 쿼리로 복구된 타임스탬프를 확인할 수 있습니다.
    \end{itemize}
    \item \textbf{스트림 중지:}
    \begin{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \_ \texttt{_rescued_data} 컬럼을 활용하여 잘못된 데이터를 식별하고 복구하는 방법을 이해하는가?
...
\begin{itemize}
    ...
    \item \textbf{Q: \texttt{_rescued_data} 컬럼에 데이터가 계속 쌓이면 어떻게 해야 하나요?}
    \textbf{A:} \texttt{_rescued_data} 컬럼에 데이터가 지속적으로 쌓인다는 것은 업스트림 데이터 소스에서 스키마가 예상과 다르게 변경되었거나, 데이터 품질 문제가 반복적으로 발생하고 있다는 신호입니다. 이 경우, 업스트림 팀과 소통하여 데이터 소스의 스키마 또는 데이터 품질 문제를 해결하거나, 파이프라인의 스키마 진화 로직을 업데이트해야 합니다.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Type 2 (새 레코드):} 이력 유지 (\texttt{start_date}, \texttt{end_date}, \texttt{is_current} 사용).
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{스키마 관리:}
    \begin{itemize}
        \item \textbf{\texttt{_rescued_data} 컬럼:} 파싱 실패 데이터 저장.
