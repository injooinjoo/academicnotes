...
% PDF Metadata
\hypersetup{
  pdftitle={CSCI103 Lecture 7: 데이터 파이프라인 운영화},
...
\fancyhead[L]{CSCI103 Lecture 7: 데이터 파이프라인 운영화}
\fancyfoot[C]{\thepage}
\fancyfoot[R]{\today}
...
\newpage
\section*{\centering 개요}

\begin{tcolorbox}[summarybox]
이 문서는 CSCI103 Lecture 7의 강의 내용을 바탕으로 데이터 파이프라인을 효과적으로 운영하는 방법을 다룹니다.
데이터 과학자가 개발한 프로토타입을 신뢰할 수 있는 자동화된 파이프라인으로 전환하는 과정에 초점을 맞춥니다.
주요 내용은 요구사항 정의, 파이프라인 설계 원칙, 구성 요소, 데이터 유효성 검사 및 품질 관리, 거버넌스, 확장성 및 탄력성입니다.
특히 Databricks의 LakeFlow 및 선언적 파이프라인(Declarative Pipelines)을 활용하여 복잡한 운영 작업을 단순화하고 비용 효율성을 높이는 방법을 상세히 설명합니다.
\end{tcolorbox}

\newpage
\section*{\centering 용어 정리}
...
\begin{itemize}
    ...
    \item \textbf{보고 및 분석:} 생성해야 할 보고서 유형, 과거 데이터 사용 방식.
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item \textbf{페더레이션 (Federation):} 여러 데이터셋에 걸쳐 접근할 수 있도록 합니다.
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item \textbf{접근 제어 (Access Control):} 행 및 열 수준 접근 제어, 데이터 마스킹 및 익명화(PII), 감사 로그(Audit Logs)를 통해 데이터 보안을 유지합니다.
        \end{itemize}
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{스토리지와 컴퓨팅 분리 (Separate Storage from Compute):}
        \begin{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{ELT:} 데이터를 원본에서 \textbf{추출(Extract)}하고, 대상 시스템에 먼저 \textbf{로드(Load)}한 다음, 대상 시스템 내에서 \textbf{변환(Transform)}하는 방식입니다. 비정형 또는 반정형 데이터, 대규모 데이터 처리에 유연하며, 클라우드 데이터 웨어하우스 환경에서 많이 사용됩니다.
    ...
    \begin{itemize}
        ...
        \item \textbf{견고성:} 문제가 발생했을 때 자동으로 재시도(Retry)하여 파이프라인이 계속 작동하도록 돕습니다.
    \end{itemize}
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{완전성 (Completeness):} 누락되거나 부분적인 데이터가 없는가?
...
\begin{itemize}
    ...
    \item \textbf{해결:} `date_of_birth`라는 새 컬럼을 추가하고, 기존 `birth_date` 컬럼을 `date` 형식으로 캐스팅하여 월/일/년 정보만 포함하도록 합니다.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{데이터 발견 가능성 (Discoverability):} 데이터 카탈로그를 통해 유용한 데이터와 데이터 제품을 쉽게 찾을 수 있는가?
...
\begin{itemize}
    ...
    \item \textbf{수직적 확장 (Scale Up):} 더 큰 노드(예: 더 강력한 CPU, 더 많은 RAM을 가진 서버)를 사용하여 용량을 늘리는 방식입니다. 한계가 있으며 비용이 많이 들 수 있습니다.
...
\begin{itemize}
    ...
    \item \textbf{재사용 가능한 패턴:} 데이터 엔지니어가 반복적으로 작성해야 했던 일반적인 운영 패턴(예: 데이터 품질 검사, 스키마 변경 처리)을 엔진에 내장하여 개발 시간을 단축합니다.
...
\begin{itemize}
    ...
    \item \textbf{오케스트레이션 (Orchestration):} LakeFlow는 전체 파이프라인을 분석하여 데이터 흐름, 종속성, 병렬 실행 가능성을 파악하고 최적의 실행 계획을 수립합니다.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{스키마 진화 (Evolve Schema):} 스키마 변경을 허용하고, 새로운 컬럼 등을 자동으로 통합합니다. 이는 소스 데이터가 새로운 정보를 제공할 것으로 예상될 때 유용합니다. 다양한 병합 전략을 지정할 수 있습니다.
\end{itemize}
...
\newpage
\section*{\centering 실습/코드/오류와 해결}
...
(spark.readStream
  ...
  .format("delta")
  .outputMode("append")
  .option("checkpointLocation", checkpoint_path) # 올바른 체크포인트 경로 사용
...
\newpage
\section*{\centering FAQ (자주 묻는 질문)}
...
\newpage
\section*{\centering 빠르게 훑어보기 (1페이지 요약)}
...
\begin{itemize}
    ...
    \item \textbf{접근 민주화:} 조직 전체의 데이터 기반 의사결정 지원.
