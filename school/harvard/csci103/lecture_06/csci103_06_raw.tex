...
% PDF 메타데이터 설정
\hypersetup{
  pdftitle={CSCI103 Lecture 6: BI 분석 및 데이터 시각화},
...
\fancyhead[L]{CSCI103 Lecture 6: BI 분석 및 데이터 시각화} % 왼쪽 헤더
...
\begin{tcolorbox}[mybox={데이터 저장소 진화의 여정}]
...
\begin{tcolorbox}[mywarning={지속적인 진화}]
...
\begin{tcolorbox}[mybox={핵심 요약: 레이크하우스의 등장 배경}]
...
\begin{tcolorbox}[myexample={레이크하우스의 장점}]
...
\begin{tcolorbox}[mybox={핵심 요약: 데이터 품질 관리}]
...
\begin{tcolorbox}[myexample={올림픽 메달 비유}]
...
\begin{tcolorbox}[mybox={핵심 요약: 데이터 관리의 함정}]
...
\begin{tcolorbox}[mybox={핵심 요약: 데이터 접근성 향상}]
...
\begin{tcolorbox}[mybox={핵심 요약: 미래를 보는 수정구슬}]
...
\begin{itemize}
    ...
    \item \textbf{셀프 서비스 BI 및 클라우드 컴퓨팅 (AWS):} Amazon AWS의 등장으로 클라우드 기반 컴퓨팅이 확산되며 BI 접근성이 향상.
...
\begin{tcolorbox}[mywarning={데이터 엔지니어링의 중요성}]
...
\begin{tcolorbox}[mybox={핵심 요약: 과거 vs. 미래}]
...
\begin{tcolorbox}[mybox={핵심 요약: 데이터에서 의사결정까지}]
...
\begin{itemize}
    ...
    \item \textbf{중앙 집중식 데이터 카탈로그 (Centralized Data Catalogs):} 데이터 위치 및 메타데이터 관리.
...
\begin{tcolorbox}[mybox={핵심 요약: 비즈니스 통찰력의 전달자}]
...
\begin{tcolorbox}[mybox={핵심 요약: 데이터 구조의 언어}]
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{대리 키 (Surrogate Keys):} 비즈니스 의미가 없는 인공적인 키.
...
\begin{tcolorbox}[mybox={핵심 요약: 데이터의 청사진}]
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item \textbf{링크 (Links):} 다양한 허브 간의 관계(기본 키와 외래 키)를 정의합니다.
...
\begin{tcolorbox}[mybox={핵심 요약: 통합된 데이터 여정}]
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\textwidth]{databricks_architecture_conceptual.png} % 이미지 파일이 없으므로 개념적 설명으로 대체
...
\begin{tcolorbox}[myexample={페더레이션과 ETL의 차이}]
...
\begin{tcolorbox}[mybox={핵심 요약: 레이크하우스의 BI 엔진}]
...
\begin{itemize}
    ...
    \item \textbf{성능:} Photon 엔진과 예측 IO(Predictive IO)를 통해 고성능을 제공합니다.
    ...
    \item \textbf{Python UDFs:} SQL 내에서 Python UDF를 사용할 수 있습니다.
...
\begin{tcolorbox}[mybox={핵심 요약: BI 시스템의 체력과 유연성}]
...
\begin{tcolorbox}[mywarning={쿼리 프로파일의 중요성}]
...
\begin{tcolorbox}[mybox={핵심 요약: 데이터 레이크에 물 채우기}]
...
\begin{tcolorbox}[mybox={핵심 요약: 대시보드로 데이터 스토리텔링}]
...
\begin{tcolorbox}[mybox={핵심 요약: 자연어로 데이터와 대화하기}]
...
\begin{tcolorbox}[myexample={Genie의 강점}]
...
\begin{tcolorbox}[mybox={핵심 요약: SQL 분석가의 작업대}]
...
\begin{tcolorbox}[mybox={핵심 요약: 쿼리 성능 진단서}]
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{전체 스캔 (Full Scan):} 테이블 전체를 스캔했는지 여부.
...
\begin{tcolorbox}[mybox={핵심 요약: 데이터 이상 감지 시스템}]
...
\begin{tcolorbox}[mybox={학습 및 복습 체크리스트}]
\begin{itemize}
    ...
    \item \textbf{BI vs. BA:} BI와 BA의 주요 질문, 분석 유형, 목표 차이를 설명할 수 있는가?
    ...
    \item \textbf{페더레이션 vs. ETL:} 데이터 페더레이션과 ETL의 차이점(특히 데이터 소유권)을 명확히 설명할 수 있는가?
...
\begin{tcolorbox}[mybox={시험 대비 전략 체크리스트}]
\begin{itemize}
    \item \textbf{개념 정의 암기:} 모든 주요 용어의 정의와 원어를 정확히 암기했는가?
    \item \textbf{비교 분석:} 유사한 개념(예: Data Lake vs. DW vs. Lakehouse, BI vs. BA, Data Mesh vs. Fabric, Federation vs. ETL) 간의 차이점을 표로 정리하고 설명할 수 있는가?
...
\begin{tcolorbox}[mybox={FAQ: 궁금증 해소}]
\begin{itemize}
...
\begin{tcolorbox}[mybox={CSCI103 Lecture 6 핵심 요약}]
...
\begin{itemize}
    ...
    \item \textbf{서버리스:} 즉시 컴퓨팅 제공, 높은 동시성 및 확장성.
