...
\fancyhead[R]{\small\textit{Module 02}}
\fancyfoot[C]{\thepage}
...
\hypersetup{
    pdftitle={CSCI E-103: Introduction to the Intellectual Enterprises of CS - Module 02},
    pdfauthor={강의 노트},
...
& \textit{예시:} \code{CUSTOMER} 테이블(고객번호:PK, 이름), \code{PRODUCT} 테이블(제품ID:PK, 이름), \\
& \code{CUSTOMER_ADDRESS} 조인 테이블(고객번호:FK, 주소ID:FK, 사용코드:CHAR(2)) \\
\bottomrule
\end{tabular}
\end{adjustbox}
...
\begin{itemize}
    ...
    \item \textbf{컬럼 수 및 사용량:} 저장되는 컬럼 수와 분석에 사용되는 컬럼 수를 고려해야 합니다. \code{SELECT *} 쿼리는 불필요한 데이터를 읽어 성능을 저하시킬 수 있습니다.
...
\begin{itemize}
    ...
    \item \textbf{업서트 (Upsert) 작업 간소화:} 기존 행을 업데이트하거나 새 행을 삽입하는 \code{MERGE} 작업을 단일 명령으로 처리할 수 있습니다.
    \item \textbf{배치 및 스트리밍 통합:} 배치 처리와 스트리밍 처리를 동일한 테이블에서 지원하여 파이프라인 복잡성을 줄입니다.
    \item \textbf{트랜잭션 로그 (Transaction Log):} 모든 변경 사항을 기록하여 테이블의 상태를 재구성하고, 데이터 일관성을 유지합니다. (\code{\_delta\_log} 디렉토리에 저장)
\end{itemize}
...
스파크 데이터프레임에서 \code{describe()} 또는 \code{summary()} 명령을 사용하여 데이터 세트의 통계적 세부 정보를 빠르게 확인할 수 있습니다.
...
\begin{enumerate}
    ...
    \item \textbf{노트북 가져오기:}
    \begin{itemize}
        \item \code{Workspace} -> \code{Home} (또는 원하는 폴더)로 이동합니다.
        \item 폴더를 생성(\code{Labs} 등)하고 해당 폴더 안에서 \code{Import} 버튼을 클릭합니다.
        \item 다운로드한 zip 파일을 업로드하여 가져옵니다.
    \end{itemize}
    \item \textbf{노트북 확인:} 가져온 \code{lab01} 폴더 안에 \code{pispark.mml}과 \code{scikitlearn} 두 개의 노트북이 있는지 확인합니다.
\end{enumerate}

\begin{tcolorbox}[cautionbox, title=중요: 서버리스 환경 제약사항]
데이터브릭스 무료 에디션(Free Edition)은 서버리스(Serverless) 컴퓨팅 환경을 사용합니다. 현재 서버리스 환경에서는 Spark ML (분산 학습 라이브러리)이 지원되지 않습니다. 따라서 \code{pispark.mml} 노트북은 실행되지 않거나 오류가 발생할 수 있습니다. \textbf{반드시 \code{scikitlearn} 버전의 노트북을 사용해야 합니다.} \code{scikitlearn}은 단일 노드 머신러닝 라이브러리를 사용하며, PySpark Pandas를 통해 분산 Spark DataFrame을 단일 노드 Pandas DataFrame으로 변환하여 처리합니다.
\end{tcolorbox}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \code{lab01} 스키마와 \code{input}, \code{output} 볼륨을 생성합니다. 이는 데이터 저장 및 관리를 위한 기본 환경을 설정하는 과정입니다.
        \item 
...
CREATE VOLUME IF NOT EXISTS output;
    ...
    \begin{itemize}
        \item 공개 GitHub 저장소에서 주택 가격 CSV 파일을 다운로드하여 \code{input} 볼륨에 저장합니다.
        \item 
...
wget -O /Volumes/unity_catalog_demo/lab01/input/housing.csv https://raw.githubusercontent.com/apache/spark/master/examples/src/main/resources/housing.csv
    ...
    \begin{itemize}
        \item \code{dbutils.fs.cp} 명령을 사용하여 \code{input} 볼륨에서 \code{output} 볼륨으로 파일을 복사합니다. 이는 파일 이동을 보여주는 예시입니다.
        \item 
...
housing_path = "/Volumes/unity_catalog_demo/lab01/output/housing.csv"
    ...
    \begin{itemize}
        \item CSV 파일을 Spark DataFrame으로 읽어들이고, \code{display()} 명령으로 데이터를 확인합니다. \code{df.describe().show()}를 추가하여 통계 요약을 볼 수도 있습니다.
        \item 
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item 기존 데이터프레임에 \code{date} (Unix 타임스탬프 변환)와 \code{zipcode} (문자열 캐스팅) 컬럼을 추가합니다.
        \item 
...
display(df_augmented)
    ...
    \begin{itemize}
        \item 전처리된 데이터를 \code{table_housing}이라는 Delta 테이블로 저장합니다. Delta Lake는 데이터브릭스의 기본 형식이며, ACID 트랜잭션, 스키마 진화 등의 기능을 제공합니다.
        \item 
...
df_augmented.write.format("delta").mode("overwrite").saveAsTable("table_housing")
    ...
    \item \textbf{테이블 탐색:}
    \begin{itemize}
        \item \code{DESCRIBE EXTENDED table_housing} 명령으로 테이블의 상세 정보를 확인합니다. \code{provider}가 \code{delta}인지 확인합니다.
        \item 
...
DESCRIBE EXTENDED table_housing;
    ...
    \begin{itemize}
        ...
        \item 가격과 주거 면적(\code{sqft_living}) 간의 상관관계를 시각화합니다.
        \item \textbf{시각화 방법:} 쿼리 결과 셀 옆의 \code{+} 버튼을 클릭하고 \code{Visualization}을 선택합니다. \code{Scatter plot}을 선택하고 X축을 \code{sqft_living}, Y축을 \code{price}로 설정하면 주거 면적이 넓을수록 가격이 높아지는 경향을 확인할 수 있습니다.
        \item 
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Pandas와 Spark의 호환성을 위해 \code{spark.sql.ansi.enabled} 설정을 \code{false}로 변경합니다.
        \item 
...
spark.conf.set("spark.sql.ansi.enabled", False)
    ...
    \begin{itemize}
        \item \code{zipcode} 컬럼을 \code{LabelEncoder}를 사용하여 고유한 숫자 값으로 인코딩합니다.
...
display(df_final)
    ...
    \begin{itemize}
        \item \code{randomSplit()} 함수를 사용하여 데이터를 훈련 세트(80\%)와 테스트 세트(20\%)로 나눕니다. 재현성을 위해 \code{seed} 값을 설정합니다.
        \item 
...
train_df, test_df = df_final.randomSplit([0.8, 0.2], seed=123)
    ...
    \begin{itemize}
        \item Scikit-learn의 \code{LinearRegression} 모델을 사용하여 훈련 데이터에 모델을 학습시킵니다.
        \item 모델의 계수(coefficients)와 절편(intercept)을 출력하고, \code{mean_squared_error} 및 \code{r2_score}를 사용하여 모델 성능을 평가합니다.
        \item 
...
display(df_predictions)
    ...
    \begin{itemize}
        \item 예측 결과를 \code{table_predictions}라는 Delta 테이블로 저장하여 영구적으로 보존합니다.
        \item 
...
df_predictions.write.format("delta").mode("overwrite").saveAsTable("table_predictions")
    ...
    \begin{itemize}
        ...
        \item \textbf{시각화 방법:} 쿼리 결과 셀 옆의 \code{+} 버튼을 클릭하고 \code{Visualization}을 선택합니다.
        \begin{itemize}
            \item \textbf{히스토그램:} \code{Histogram}을 선택하고 X축을 \code{residual_error}로 설정합니다. \code{Number of bins}를 조정하여 오차 분포를 확인합니다. 대부분의 오차가 0 근처에 집중되어 있음을 볼 수 있습니다.
            \item \textbf{파이 차트:} \code{Pie chart}를 선택하고 X축을 \code{RMSSE_multiple}, Y축을 \code{count}로 설정합니다. 이를 통해 RMSSE 1 이내에 들어오는 예측의 비율(예: 80\%)과 RMSSE 2 이내에 들어오는 예측의 비율(예: 96\%)을 확인할 수 있습니다.
        \end{itemize}
        \item 
...
\begin{itemize}
    \item [ ] Lab 01 노트북(\code{scikitlearn} 버전)을 데이터브릭스 워크스페이스에 성공적으로 가져왔는가?
    \item [ ] 스키마 및 볼륨 생성 셀을 성공적으로 실행했는가?
    \item [ ] CSV 파일 다운로드 및 복사 셀을 성공적으로 실행했는가?
    \item [ ] Spark DataFrame으로 데이터를 로드하고 \code{display()}로 확인했는가?
    \item [ ] \code{date} 및 \code{zipcode} 컬럼을 추가하고 데이터프레임을 확장했는가?
    \item [ ] 확장된 데이터를 \code{table_housing} Delta 테이블로 저장했는가?
    \item [ ] \code{DESCRIBE EXTENDED} 명령으로 \code{table_housing}의 Delta 형식을 확인했는가?
    \item [ ] 가격과 주거 면적(\code{sqft_living}) 간의 상관관계를 산점도(scatter plot)로 시각화했는가?
    \item [ ] \code{spark.sql.ansi.enabled} 설정을 \code{False}로 변경했는가?
    \item [ ] \code{zipcode} 컬럼을 \code{LabelEncoder}로 인코딩하고 특징을 조합했는가?
    \item [ ] 데이터를 훈련 세트와 테스트 세트로 \code{randomSplit()}을 사용하여 분할했는가?
    \item [ ] Scikit-learn \code{LinearRegression} 모델을 훈련 데이터에 학습시켰는가?
    \item [ ] 모델의 \code{RMSE}와 \code{R2 Score}를 확인했는가?
    \item [ ] 학습된 모델로 테스트 데이터에 대한 예측을 수행하고 \code{prediction} 컬럼을 추가했는가?
    \item [ ] 예측 결과를 \code{table_predictions} Delta 테이블로 저장했는가?
    \item [ ] 잔차 오차(\code{residual_error})의 분포를 히스토그램으로 시각화했는가?
...
\begin{tcolorbox}[summary, title=Q: 과제에서 데이터베이스 이름을 \code{\_assignment\_one} 대신 \code{username\_assignment\_one}으로 해야 한다고 되어 있는데, 어떻게 해야 하나요?]
\textbf{A:} 현재는 자신의 환경에서 작업하므로, \code{\_assignment\_one}과 같은 이름으로 진행해도 됩니다. \code{username\_assignment\_one}은 과거에 모든 사용자가 공유 환경을 사용할 때 충돌을 피하기 위한 지침이었습니다. 강사가 과제 지침을 수정할 예정이므로, 현재 제공된 코드의 이름으로 따라가세요.
\end{tcolorbox}
...
\textbf{A:} 아니요, 데이터 자산은 Google Drive의 \code{assignment 01} 폴더 안에 있습니다. \code{assignment 01} 폴더에는 과제 노트북과 \code{data} 디렉토리가 있습니다. \code{data} 디렉토리 안에 \code{blog.json}, \code{people-10m} (parquet 파일), \code{names} 데이터셋이 있습니다. 이 파일들을 다운로드하여 데이터브릭스 볼륨에 업로드해야 합니다.
\end{tcolorbox}

\begin{tcolorbox}[summary, title=Q: 데이터브릭스 볼륨에 데이터를 업로드하는 구체적인 절차는 무엇인가요?]
\textbf{A:}
\begin{enumerate}
    \item 데이터브릭스 워크스페이스에서 \code{Catalog}로 이동합니다.
    \item 과제용으로 생성한 카탈로그와 스키마를 선택합니다.
    \item \code{Volumes} 탭에서 \code{Create Volume}을 클릭하여 관리형 볼륨을 생성합니다.
    \item 생성된 볼륨 안에 \code{people-10m}, \code{names}와 같은 디렉토리를 생성합니다.
    \item 각 디렉토리로 이동하여 \code{Upload to this volume}을 클릭하고, 로컬에 다운로드한 해당 데이터 파일(예: \code{blog.json}, \code{parquet} 파일)을 업로드합니다.
\end{enumerate}
\end{tcolorbox}
...
\begin{itemize}
    ...
    \item \textbf{도구:} Spark \code{describe()}, \code{summary()}.
\end{itemize}
\end{tcolorbox}
