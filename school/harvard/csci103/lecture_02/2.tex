...
\begin{examplebox}
  ...
  \begin{itemize}
    ...
    \item \textbf{3. 물리적 모델 (PostgreSQL 기준):}
      \begin{lstlisting}[language=SQL, caption=물리적 모델 예시 (SQL), label=lst:physical_model]
...
\begin{examplebox}
  ...
  \begin{lstlisting}[language=json, caption=임베디드 모델 예시, label=lst:embedded]
{
  "post_id": "p123",
  "title": "My First Post",
  "content": "Hello world!",
  ...
  \begin{lstlisting}[language=json, caption=참조 모델 예시 (Posts), label=lst:referenced_post]
(*@\textit{// Posts Collection}@*)
{
  ...
  \end{lstlisting}
  \begin{lstlisting}[language=json, caption=참조 모델 예시 (Comments), label=lst:referenced_comments]
