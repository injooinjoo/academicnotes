...
\hypersetup{
  ...
  filecolor=magenta,
  urlcolor=cyan,
  pdftitle={CSCI103 Lecture 10: 데이터 및 모델 고려사항, AutoML, 대규모 사기 탐지},
...
\fancyhead[L]{CSCI103 Lecture 10} % 왼쪽 헤더
...
% tcolorbox 스타일 정의
\definecolor{myblue}{rgb}{0.9,0.95,1.0}
\definecolor{mygreen}{rgb}{0.9,1.0,0.9}
\definecolor{myred}{rgb}{1.0,0.9,0.9}
...
본 문서는 CSCI103 Lecture 10 강의 자료를 바탕으로 작성되었습니다.
강의는 데이터와 모델에 대한 심층적인 고려사항을 다루며, 특히 모델 편향, 데이터 불균형, 익명성 문제를 강조합니다.
AutoML의 개념과 Databricks의 글래스박스(Glassbox) AutoML 구현을 통해 모델 개발의 효율성과 투명성을 탐구합니다.
또한, 대규모 신용카드 사기 탐지 시스템의 실제 사례를 통해 복잡한 비즈니스 요구사항과 기술적 해결책을 살펴봅니다.
이 문서는 비전공자도 쉽게 이해할 수 있도록 핵심 개념, 원리, 절차, 그리고 실제 적용 사례를 상세히 설명합니다.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \end{itemize}
        변환기는 `transform` 메서드를, 추정기는 `fit` 또는 `predict` 메서드를 호출합니다.
    \end{itemize}
...
\begin{itemize}
    ...
    \begin{figure}[h!]
        \centering
        \includegraphics[width=0.8\textwidth]{bias_variance_tradeoff.png} % 가상의 이미지 파일명
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{기술적 정확한 설명}: `value_counts()`와 같은 함수를 사용하여 각 클래스의 데이터 포인트 수를 확인하면, 다수 클래스(majority class)와 소수 클래스(minority class)를 식별할 수 있습니다. 불균형의 정도는 경미할 수도 있고 매우 심각할 수도 있습니다.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \end{enumerate}
            이 방법은 단순히 기존 샘플을 복제하는 것보다 더 다양한 데이터를 생성하여 모델의 일반화 성능을 향상시킵니다. `imblearn` 라이브러리에서 `SMOTE` 함수를 사용할 수 있습니다.
            \begin{figure}[h!]
                \centering
                \includegraphics[width=0.6\textwidth]{smote_example.png} % 가상의 이미지 파일명
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\textwidth]{fraud_detection_architecture.png} % 가상의 이미지 파일명
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{훈련 데이터셋 지정}: 3단계 이름 공간(카탈로그.스키마.테이블)을 사용하여 훈련 데이터를 선택합니다. (예: `churn` 데이터셋)
        \item \textbf{예측 컬럼 지정}: 모델이 예측할 목표 컬럼을 선택합니다. (예: `churn`이 'yes'인지 'no'인지)
        \item \textbf{실험 이름 부여}: 실험에 고유한 이름을 지정합니다. (예: `churn-am`)
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item `imblearn` (imbalanced-learn) 라이브러리를 설치합니다.
...
\end{lstlisting}
    ...
    \begin{itemize}
        ...
        \item \textbf{기술적 정확한 설명}: `RandomOverSampler`를 사용하여 소수 클래스(사기)의 샘플을 복제하여 훈련 세트의 클래스 분포를 균등하게 만듭니다.
    \end{itemize}
...
\end{lstlisting}
    ...
    \begin{itemize}
        ...
        \item \textbf{기술적 정확한 설명}: `RandomUnderSampler`를 사용하여 다수 클래스(비사기)의 샘플을 무작위로 제거하여 훈련 세트의 클래스 분포를 균등하게 만듭니다.
    \end{itemize}
...
\end{lstlisting}
    ...
    \begin{itemize}
        ...
        \item \textbf{기술적 정확한 설명}: `SMOTE`를 사용하여 소수 클래스의 샘플을 복제하는 대신, 통계적 특성을 고려한 합성 샘플을 생성하여 훈련 세트의 클래스 분포를 균등하게 만듭니다.
    \end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{AutoML}: 블랙박스 AutoML과 글래스박스 AutoML의 차이점을 설명할 수 있는가?
...
\begin{itemize}
    ...
    \item \textbf{Q: 모델의 정확도가 99\%인데도 문제가 될 수 있나요?}
...
\begin{itemize}
    ...
    \item \textbf{활용}: 데이터 유효성 검사, EDA 자동화, 시민 데이터 과학자 지원.
\end{itemize}
