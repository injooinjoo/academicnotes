...
\lstset{
  ...
  frameround=tttt, % 프레임 모서리 둥글게
  rulecolor=\color{gray!40}, % 프레임 색상
  backgroundcolor=\color{gray!5}, % 배경색
  keywordstyle=\color{blue}\bfseries, % 키워드 스타일
  identifierstyle=\color{black}, % 식별자 스타일
  commentstyle=\color{green!50!black}, % 주석 스타일
  stringstyle=\color{red!70!black}, % 문자열 스타일
  showstringspaces=false, % 문자열 공백 표시 안 함
  tabsize=2, % 탭 크기
  caption=\lstname, % 캡션 이름
  label=\lstname % 라벨 이름
}

% 하이퍼링크 설정
\hypersetup{
  pdftitle={Banking and Financial Institutions Module 3: Technology Strategy},
  pdfauthor={Professor Rustom Manouchehri Irani / UIUC Faculty},
...
\fancyhead[L]{\textbf{Banking and Financial Institutions Module 3}} % 왼쪽 헤더
\fancyhead[R]{\textbf{Technology Strategy}} % 오른쪽 헤더
\fancyfoot[C]{\thepage} % 중앙 푸터 (페이지 번호)
\fancyfoot[R]{\today} % 오른쪽 푸터 (날짜)
\renewcommand{\headrulewidth}{0.4pt} % 헤더 라인 두께
\renewcommand{\footrulewidth}{0.4pt} % 푸터 라인 두께
...
\begin{titlepage}
    \centering
    \vspace*{1cm}
    {\Huge\textbf{Banking and Financial Institutions Module 3}}\\[1cm]
    {\Large\textbf{기술 전략 (Technology Strategy)}}\\[1.5cm]
    {\Large Professor Rustom Manouchehri Irani}\\[2cm]
    {\Large FIN-571: Financial Institutions \& Monetary Policy}\\[0.5cm]
...
\begin{tcolorbox}[summarybox, title=\textbf{학습 목표 요약}]
\begin{itemize}
...
\begin{tcolorbox}[mybox, title=\textbf{핵심 요약: 은행 자기자본}]
...
\begin{tcolorbox}[summarybox, title=\textbf{자기자본 요약}]
...
\begin{tcolorbox}[mybox, title=\textbf{핵심 요약: 은행 성과 측정}]
...
\begin{tcolorbox}[summarybox, title=\textbf{은행 성과 측정 요약}]
...
\begin{tcolorbox}[mybox, title=\textbf{핵심 요약: 위험의 정의와 종류}]
...
\begin{tcolorbox}[summarybox, title=\textbf{위험 개요 요약}]
...
\begin{tcolorbox}[mybox, title=\textbf{핵심 요약: 신용 위험}]
...
\begin{tcolorbox}[summarybox, title=\textbf{신용 위험 요약}]
...
\begin{tcolorbox}[mybox, title=\textbf{핵심 요약: 금리 위험}]
...
\begin{tcolorbox}[summarybox, title=\textbf{금리 위험 요약}]
...
\begin{tcolorbox}[mybox, title=\textbf{핵심 요약: 유동성 위험}]
...
\begin{tcolorbox}[summarybox, title=\textbf{유동성 위험 요약}]
...
\begin{tcolorbox}[mybox, title=\textbf{핵심 요약: 시장 위험}]
...
\begin{tcolorbox}[summarybox, title=\textbf{시장 위험 요약}]
...
\begin{tcolorbox}[mybox, title=\textbf{핵심 요약: 운영 위험}]
...
\begin{tcolorbox}[summarybox, title=\textbf{운영 위험 요약}]
...
\begin{enumerate}
    ...
    \item \textbf{위험 통제 및 완화 (Risk Control and Mitigation):}
        ...
        \begin{itemize}
            ...
            \item \textbf{위험 이전 (Risk Transfer):}
                \begin{itemize}
...
\begin{tcolorbox}[examplebox, title=\textbf{예시: 금리 위험 관리 절차}]
\begin{enumerate}
...
\begin{tcolorbox}[summarybox, title=\textbf{모듈 3 학습 점검 체크리스트}]
\begin{itemize}
...
\begin{tcolorbox}[mybox, title=\textbf{FAQ: 은행 위험 및 수익성}]
...
\begin{tcolorbox}[summarybox, title=\textbf{모듈 3 핵심 요약: 은행의 위험과 수익}]
\textbf{1. 은행 자본과 수익성}
\begin{itemize}
...
\begin{tcolorbox}[mybox, title=\textbf{모듈 3 학습 로드맵}]
\begin{enumerate}
