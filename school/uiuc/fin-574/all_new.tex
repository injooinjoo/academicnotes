...
% -----------------------------------------------------------------------------
% [공통 설정] 프리앰블 (모든 단원에 공통적으로 필요)
% -----------------------------------------------------------------------------
\usepackage{kotex} % 한글 지원
\usepackage[margin=25mm]{geometry} % 여백 설정
\usepackage{hyperref} % 하이퍼링크
\usepackage[table,xcdraw]{xcolor} % 색상 지원
...
\newtcolorbox{summarybox}[1]{
  colback=mainblue!5!white, colframe=mainblue, fonttitle=\bfseries, title=#1
}
\newtcolorbox{alertbox}[1]{
  colback=alertred!5!white, colframe=alertred, fonttitle=\bfseries, title=#1
}

% 헤더/푸터
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\textbf{FIN574 요약 노트}}
\fancyhead[R]{Part 1. 기초 및 시장 원리}
\fancyfoot[C]{\thepage}
...
시장은 **물건을 사는 사람(소비자)**과 **파는 사람(생산자)**이 만나는 곳입니다. 이 둘의 줄다리기가 가격을 결정합니다.

\subsection{1. 수요 (Demand)}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item 원인: 해당 재화의 **가격**이 변할 때.
        \item 결과: 그래프 **곡선 위에서 점이 이동**합니다. (가격 인상 $\rightarrow$ 구매 감소)
    \end{itemize}
    \item \textbf{수요의 변화 (Demand):} 
    \begin{itemize}
        \item 원인: 가격 이외의 요인 (유행, 소득, 대체재 가격 등).
        \item 결과: **그래프 자체가 왼쪽이나 오른쪽으로 이동**합니다. (예: 건강 연구 발표 $\rightarrow$ 토마토 수요 곡선 우측 이동)
    \end{itemize}
\end{itemize}
\end{alertbox}

\subsection{2. 공급 (Supply)}
...
\subsection{3. 시장 균형 (Market Equilibrium)}
...
\newtcolorbox{examplebox}[1]{
  colback=examplegreen!5!white, colframe=examplegreen, fonttitle=\bfseries, title=#1
}
...
\section*{Part 2. 기업의 생산과 비용 구조}

\begin{summarybox}{단원 개요}
기업의 목표는 **이윤 극대화(Profit Maximization)**입니다. \\
이윤은 '번 돈(매출)'에서 '쓴 돈(비용)'을 뺀 것입니다. \\
이 단원에서는 물건을 만들 때 비용이 어떤 규칙으로 발생하는지, \\
왜 많이 만들수록 개당 비용이 변하는지를 분석합니다.
\end{summarybox}
...
\subsection{1. 단기와 장기의 구분 (Time Frames)}
경제학에서 시간은 달력의 날짜가 아니라, **'생산 요소를 바꿀 수 있는가'**로 구분합니다.
\begin{itemize}
    \item \textbf{단기 (Short Run):} 
    \begin{itemize}
        \item 공장 크기나 기계 설비 같은 **고정 요소(Capital, K)**는 바꿀 수 없습니다.
        \item 오직 **가변 요소(Labor, L)**인 직원 수나 재료만 조절 가능합니다.
    \end{itemize}
    \item \textbf{장기 (Long Run):} 
    \begin{itemize}
        \item 공장을 확장하거나 폐쇄하는 등 **모든 요소**를 바꿀 수 있는 충분한 시간입니다.
    \end{itemize}
\end{itemize}

\subsection{2. 수확 체감의 법칙 (Law of Diminishing Marginal Product)}
...
\begin{itemize}
    ...
    \item 하지만 10명이 넘어가면? 서로 동선이 꼬이고 부딪혀서 **1명당 생산 효율이 떨어집니다.**
    \item 이를 **한계 생산 체감**이라고 하며, 비용이 급격히 증가하는 원인이 됩니다.
\end{itemize}
\end{examplebox}
...
\subsection{1. 한계 비용 (Marginal Cost, MC)}
...
\subsection{2. 평균 비용 (Average Total Cost, ATC)}
...
\fancyhf{}
\fancyhead[L]{\textbf{FIN574 요약 노트}}
\fancyhead[R]{Part 3. 이윤 극대화와 완전 경쟁}
\fancyfoot[C]{\thepage}
...
\section*{Part 3. 이윤 극대화와 완전 경쟁 시장}

\begin{summarybox}{단원 개요}
모든 기업은 '이윤 극대화'를 위해 움직입니다. 이 단원에서는 기업이 생산량을 결정하는 **절대 규칙($MR=MC$)**을 배웁니다. \\
또한, 가장 이상적인 시장 형태인 **'완전 경쟁 시장'**에서 가격이 어떻게 형성되고, 경쟁이 기업의 이윤을 어떻게 0으로 수렴시키는지 분석합니다.
\end{summarybox}
...
\subsection{2. 최적 생산량 결정 규칙 ($MR = MC$)}
...
\subsection{2. 완전 경쟁 시장의 4가지 조건}
이 조건들이 모두 충족되어야 '완전 경쟁'이라고 부릅니다. 현실에서는 농산물 시장이 가장 비슷합니다.
...
\subsection{3. 가격 수용자 (Price Taker)}
...
\begin{itemize}
    ...
    \item **결론:** 기업은 시장 가격($P$)을 그대로 받아들입니다. ($P = MR$)
\end{itemize}
...
\subsection{1. 진입과 퇴출 (Entry and Exit)}

시장의 자동 조절 기능입니다.
\begin{itemize}
    \item **상황 A (초과 이윤 발생):** 시장 가격이 높아서 기존 기업들이 돈을 법니다.
        \begin{itemize}
            \item $\rightarrow$ 소문을 듣고 새로운 기업들이 진입합니다.
            \item $\rightarrow$ 공급 곡선이 오른쪽으로 이동(증가)합니다.
            \item $\rightarrow$ 가격이 떨어져서 이윤이 줄어듭니다.
        \end{itemize}
    \item **상황 B (손실 발생):** 시장 가격이 낮아서 기업들이 손해를 봅니다.
        \begin{itemize}
...
\subsection{2. 장기 균형의 결론: 경제적 이윤 = 0}
진입과 퇴출이 멈추는 시점은 언제일까요? 바로 **'초과 이윤이 0'**이 될 때입니다.

\begin{alertbox}{오해 금지: 이윤이 0이면 사장님은 굶나요?}
\textbf{아닙니다!} 경제학에서 말하는 비용에는 **'기회비용(사장님의 월급)'**이 포함되어 있습니다.
\begin{itemize}
    \item **회계적 이윤:** 장부상 남는 돈. (당연히 플러스여야 함)
    \item **경제적 이윤:** 회계적 이윤 - 기회비용.
    \item **결론:** 경제적 이윤이 0이라는 말은, "다른 사업을 하는 것만큼은 딱 벌고 있다(본전 이상은 한다)"는 뜻입니다. 떼돈을 벌지는 못하지만, 망하지도 않는 안정적인 상태입니다.
\end{itemize}
\end{alertbox}
...
\fancyhf{}
\fancyhead[L]{\textbf{FIN574 요약 노트}}
\fancyhead[R]{Part 4. 독점과 과점 (게임 이론)}
\fancyfoot[C]{\thepage}
...
\section*{Part 4. 독점과 과점 (게임 이론)}

\begin{summarybox}{단원 개요}
경쟁자가 없거나(독점) 소수만 있는(과점) 시장은 완전 경쟁 시장과 전혀 다르게 작동합니다. \\
이 단원에서는 독점 기업이 어떻게 가격을 결정하여 사회적 손실을 초래하는지, 그리고 과점 기업들이 왜 서로 협력하지 못하고 배신하게 되는지(게임 이론)를 분석합니다.
\end{summarybox}
...
\subsection{2. 독점의 가격 결정 ($MR < P$)}
독점 기업은 **가격 설정자(Price Maker)**입니다. 하지만 마음대로 무한정 비싸게 팔 수는 없습니다. 소비자의 수요 곡선을 따라야 하기 때문입니다.
...
\subsection{3. 독점의 결과: 비효율성}


[Image of Monopoly deadweight loss graph]
...
\begin{itemize}
    ...
    \item \textbf{사중손실 (Deadweight Loss):} 거래가 이루어졌다면 사회적으로 이득이었을 부분(소비자 잉여+생산자 잉여)이 사라져 버립니다. 사회 전체의 파이 크기가 줄어듭니다.
\end{itemize}
...
\subsection{1. 특징: 상호 의존성 (Interdependence)}
완전 경쟁이나 독점과 달리, **'상대방의 행동'**이 나에게 큰 영향을 미칩니다.
\begin{itemize}
    \item 내가 가격을 내리면 경쟁사도 따라 내릴 것이고, 결국 둘 다 손해를 봅니다.
    \item 따라서 전략적 판단이 매우 중요해지며, 이를 분석하기 위해 **게임 이론**을 사용합니다.
\end{itemize}

\subsection{2. 담합과 카르텔 (Cartel)}
과점 기업들은 서로 싸우기보다 뭉쳐서 독점 기업처럼 행동하고 싶어 합니다.
\begin{itemize}
    \item \textbf{카르텔:} 기업들이 모여 생산량을 줄이고 가격을 올리기로 약속하는 것. (예: OPEC)
    \item \textbf{배신의 유인 (Cheating Incentive):} 
    \begin{itemize}
        \item "남들은 약속을 지켜서 생산을 줄일 때, 나만 몰래 생산을 늘리면 떼돈을 벌 수 있지 않을까?"
        \item 이 유혹 때문에 카르텔은 장기적으로 유지되기 어렵고 붕괴하기 쉽습니다.
    \end{itemize}
\end{itemize}
...
\subsection{1. 죄수의 딜레마 (Prisoner's Dilemma)}
...
\begin{tabular}{cc|c|c|}
 & \multicolumn{1}{c}{} & \multicolumn{2}{c}{\textbf{용의자 B}} \\
 & \multicolumn{1}{c}{} & \multicolumn{1}{c}{침묵 (협력)} & \multicolumn{1}{c}{자백 (배신)} \\ \cline{3-4}
\multirow{2}*{\textbf{용의자 A}} & 침묵 (협력) & (1년, 1년) & (20년, 0년) \\ \cline{3-4}
 & 자백 (배신) & (0년, 20년) & \cellcolor{yellow!30}\textbf{(5년, 5년)} \\ \cline{3-4}
\end{tabular}
\end{center}

\textbf{분석:}
\begin{enumerate}
    \item \textbf{최선의 결과 (Social Optimum):} 둘 다 침묵하면 (1년, 1년)으로 총 형량이 가장 적습니다.
    \item \textbf{우월 전략 (Dominant Strategy):} 
    \begin{itemize}
        \item 용의자 A의 생각: "B가 침묵하면? 나는 자백해서 0년 받는 게 이득(0<1). B가 자백하면? 나도 자백해서 5년 받는 게 이득(5<20)."
        \item 결론: 상대가 뭘 하든 **'자백'**하는 것이 무조건 유리합니다.
    \end{itemize}
    \item \textbf{내쉬 균형 (Nash Equilibrium):} 결국 둘 다 자백을 선택하여 (5년, 5년)을 살게 됩니다. 서로 협력했을 때보다 더 나쁜 결과를 맞이합니다.
\end{enumerate}
\end{examplebox}

\subsection{2. 경제학적 시사점}
\begin{itemize}
    \item 기업들도 가격을 높게 유지하기로 담합(침묵)하면 좋지만, 배신의 유인 때문에 결국 경쟁 가격(자백)으로 돌아가게 됩니다.
    \item 이를 막기 위해 정부는 **반독점법(예: 셔먼법)**을 통해 담합을 불법으로 규정하고 처벌합니다.
\end{itemize}
...
\fancyhf{}
\fancyhead[L]{\textbf{FIN574 요약 노트}}
\fancyhead[R]{Part 5. 정부 개입 및 최종 요약}
\fancyfoot[C]{\thepage}
...
\section*{Part 5. 정부 개입과 후생 경제학}

\begin{summarybox}{단원 개요}
시장은 보통 효율적이지만, 정부가 개입해야 할 때도 있습니다. \\
이 단원에서는 정부가 가격을 통제하거나 세금을 부과할 때 시장에 어떤 변화(비효율)가 생기는지 분석합니다. \\
또한 '누가 세금을 실질적으로 부담하는가'는 법이 아니라 '탄력성'이 결정한다는 사실을 배웁니다.
\end{summarybox}
...
\subsection{1. 자비로운 독재자 (Benevolent Dictator) 개념}
누가 얼마나 가져가는지(분배)는 신경 쓰지 않고, **오직 사회 전체의 이익 총합(효율성)**만 생각하는 가상의 존재입니다.
\begin{itemize}
...
자비로운 독재자가 보기에 가장 흐뭇한 상태는 **완전 경쟁 시장**입니다. \\
독점이나 정부 개입이 발생하면 거래량이 줄어들어 사회적 총잉여가 감소하는데, 이를 **사중손실(Deadweight Loss)**이라고 합니다.
\end{alertbox}
...
\subsection{1. 가격 상한제 (Price Ceiling)}
"이 가격 이상 받지 마!" (예: 임대료 규제, 티켓 가격 제한)
\begin{itemize}
    \item \textbf{조건:} 시장 균형 가격보다 **낮게** 설정해야 효과가 있음.
    \item \textbf{결과:} **초과 수요 (Shortage)** 발생.
    \item \textbf{부작용:} 사고 싶은 사람은 많은데 물건이 없습니다. 줄 서기, 암시장 형성, 품질 저하(집주인이 수리를 안 해줌)가 발생합니다.
\end{itemize}

\subsection{2. 가격 하한제 (Price Floor)}
"이 가격 이하로 주지 마!" (예: 최저임금, 농산물 가격 지지)
\begin{itemize}
    \item \textbf{조건:} 시장 균형 가격보다 **높게** 설정해야 효과가 있음.
    \item \textbf{결과:} **초과 공급 (Surplus)** 발생.
    \item \textbf{예시 (정부 치즈):} 과거 미국 정부가 우유 가격을 높게 유지하자 농가들이 우유를 너무 많이 생산했습니다. 정부가 이 남는 우유를 다 사들여서 치즈로 만들어 창고에 쌓아두고 무료로 나눠줬던 사례가 있습니다.
\end{itemize}
...
정부가 물건 하나당 세금(Excise Tax)을 부과하면 누가 그 돈을 낼까요? 법적으로는 판매자가 내라고 해도, 실제로는 소비자가 낼 수도 있습니다.

\subsection{1. 세금의 효과}
\begin{itemize}
    \item 공급자에게 세금을 부과하면, 공급 곡선이 **세금 액수만큼 위로 수직 이동**합니다.
    \item 결과적으로 **시장 가격은 오르고, 거래량은 줄어듭니다.**
    \item 줄어든 거래량 때문에 사회적 손실(사중손실)이 발생합니다.
\end{itemize}

\subsection{2. 누가 더 많이 내나? (탄력성의 싸움)}
세금 부담은 **'도망가기 힘든 사람(비탄력적인 쪽)'**이 더 많이 집니다.
...
\begin{itemize}
    ...
    \item \textbf{결과:} 정부가 담배 회사에 세금을 물려도, 회사는 담배 가격을 확 올려버립니다. 흡연자는 울며 겨자 먹기로 비싼 가격을 다 지불합니다. 즉, **소비자가 세금의 대부분을 부담**합니다.
\end{itemize}
\end{examplebox}
...
% 최종 요약 (Cheat Sheet)
% -----------------------------------------------------------------------------
\section*{[부록] 전체 강의 한 장 요약 (Cheat Sheet)}
