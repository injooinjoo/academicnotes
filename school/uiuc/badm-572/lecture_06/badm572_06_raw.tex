...
\section*{\centering 모듈 2: 추론 및 예측 통계}
\addcontentsline{toc}{section}{모듈 2: 추론 및 예측 통계}

\begin{summarybox}[title=개요: 두 그룹 비교의 통계적 이해]
...
\begin{tcolorbox}[mybox, title=저작권 정보]
...
\begin{figure}[h!]
    ...
    % \includegraphics[width=0.8\textwidth]{example-morrow-plots.png}
    \textbf{[이미지: UIUC의 모로우 플롯 (옥수수 밭 사진)]}
    \caption{UIUC의 모로우 플롯 (Morrow Plots at UIUC)}
    \label{fig:morrow_plots}
\end{figure}
\begin{tcolorbox}[mybox, title=모로우 플롯: 1세기의 학습과 통계의 뿌리]
...
\begin{tcolorbox}[mybox, title=평가 방법: '실제 차이'의 중요성]
...
\begin{tcolorbox}[mybox, title=두 평균에 대한 가설 검정 절차]
...
\begin{tcolorbox}[mybox, title=두 브랜드 비교 (1/8): 문제 정의 및 변수 설정]
배터리 두 브랜드의 평균 수명 차이
...
\begin{tcolorbox}[mybox, title=두 브랜드 비교 (2/8): 1단계 - 가설 설정]
...
\begin{tcolorbox}[mybox, title=두 브랜드 비교 (3/8): 2단계 - 유의 수준 설정]
\textbf{가설:}
\begin{itemize}
...
\begin{tcolorbox}[mybox, title=두 브랜드 비교 (4/8): 데이터 수집의 중요성]
...
\begin{tcolorbox}[mybox, title=두 브랜드 비교 (5/8): 3단계 - p-값 계산 (기본 원리)]
...
\begin{tcolorbox}[mybox, title=두 브랜드 비교 (6/8): 3단계 - p-값 계산 (두 모집단)]
\textbf{단일 모집단의 경우 검정 통계량:}
$t = \frac{\bar{x} - \mu_0}{s/\sqrt{n}}$
...
\begin{tcolorbox}[mybox, title=두 브랜드 비교 (7/8): 결과 해석 및 주의점]
...
\begin{tcolorbox}[mybox, title=두 브랜드 비교 (8/8): 4단계 - 귀무 가설 기각 여부 결정]
\textbf{가설:}
\begin{itemize}
    \item $H_0 : \mu_A - \mu_B = 0$
    \item $H_A : \mu_A - \mu_B \neq 0$
...
\begin{tcolorbox}[mybox, title=연습 문제: 가설은 무엇인가요? (선택)]
...
\begin{tcolorbox}[mybox, title=연습 문제: 풀이 (1/2) - 가설 및 유의 수준 설정]
...
\begin{tcolorbox}[mybox, title=연습 문제: 풀이 (2/2) - 결과 분석 및 결정]
...
\begin{tcolorbox}[mybox, title=차이는 얼마나 될까? (How Much is the Difference?) - 평균 차이 계산]
...
\begin{tcolorbox}[mybox, title=평균 차이에 대한 신뢰 구간 (Confidence Interval for the Mean Differences) - 공식]
...
\begin{tcolorbox}[mybox, title=차이는 얼마나 될까? (1/5) - 신뢰 구간 계산 시작]
...
\begin{tcolorbox}[mybox, title=차이는 얼마나 될까? (2/5) - 평균 차이 대입]
\textbf{Excel 결과:} (온라인/매장 판매 t-검정 표)
...
\begin{tcolorbox}[mybox, title=차이는 얼마나 될까? (3/5) - t-임계값 대입]
\textbf{Excel 결과:} (온라인/매장 판매 t-검정 표)
...
\begin{tcolorbox}[mybox, title=차이는 얼마나 될까? (4/5) - 표준 오차 계산]
\textbf{Excel 결과:} (온라인/매장 판매 t-검정 표)
...
\begin{tcolorbox}[mybox, title=차이는 얼마나 될까? (5/5) - 최종 신뢰 구간 및 해석]
\textbf{Excel 결과:} (온라인/매장 판매 t-검정 표)
...
\begin{tcolorbox}[mybox, title=Excel 워크시트 (2/9): 가설 설정 (Excel 준비)]
\textbf{브랜드 정의:}
\begin{itemize}
...
\begin{tcolorbox}[mybox, title=Excel 워크시트 (3/9): 데이터 준비 및 표본 크기]
...
\begin{tcolorbox}[mybox, title=Excel 워크시트 (4/9): Excel에서 t-검정 선택 절차]
\textbf{절차:}
\begin{enumerate}
...
\begin{tcolorbox}[mybox, title=Excel 워크시트 (5/9): t-검정 설정 입력]
\textbf{팝업 창 설정:}
\begin{itemize}
...
\begin{tcolorbox}[mybox, title=Excel 워크시트 (6/9): 결과 출력 및 초기 해석]
...
\begin{tcolorbox}[mybox, title=Excel 워크시트 (7/9): t-임계값 시각화 및 이해]
...
\begin{figure}[h!]
    \centering
    % 이미지 파일이 제공되지 않았으므로 텍스트로 대체합니다.
    % \includegraphics[width=0.7\textwidth]{example-bell-curve-two-tail.png}
...
\begin{tcolorbox}[mybox, title=Excel 워크시트 (8/9): p-값 기반 최종 결정]
...
\begin{tcolorbox}[mybox, title=Excel 워크시트 (9/9): 최종 결론 도출]
\textbf{가설:}
\begin{itemize}
...
\begin{tcolorbox}[mybox, title=온라인/매장 Excel 워크시트 (2/10): 가설 설정]
\textbf{가설:}
\begin{itemize}
...
\begin{tcolorbox}[mybox, title=온라인/매장 Excel 워크시트 (3/10): 데이터 준비]
...
\begin{tcolorbox}[mybox, title=온라인/매장 Excel 워크시트 (4/10): Excel에서 t-검정 선택]
\textbf{절차:}
\begin{enumerate}
    \item Excel에서 \textbf{데이터 (Data)} 탭으로 이동합니다.
...
\begin{tcolorbox}[mybox, title=온라인/매장 Excel 워크시트 (5/10): t-검정 설정 입력 (열 선택)]
\textbf{팝업 창 설정:}
\begin{itemize}
...
\begin{tcolorbox}[mybox, title=온라인/매장 Excel 워크시트 (6/10): 결과 분석 및 결정]
...
\begin{tcolorbox}[mybox, title=온라인/매장 Excel 워크시트 (7/10): 신뢰 구간 계산 준비]
\textbf{Excel 출력:} (온라인/매장 판매 t-검정 표)
...
\begin{tcolorbox}[mybox, title=온라인/매장 Excel 워크시트 (8/10): 표준 오차 계산]
\textbf{Excel 출력:} (온라인/매장 판매 t-검정 표)
...
\begin{tcolorbox}[mybox, title=온라인/매장 Excel 워크시트 (9/10): 오차 한계 및 신뢰 구간 계산]
\textbf{Excel 출력:} (온라인/매장 판매 t-검정 표)
...
\begin{tcolorbox}[mybox, title=온라인/매장 Excel 워크시트 (10/10): 통계적 유의성과 실질적 의미의 차이]
...
\begin{tcolorbox}[mybox, title=단측 검정 (One-tail Test) 가설 설정]
...
\begin{tcolorbox}[mybox, title=제품 테스트 (2/5): 가설 설정 (새 메뉴)]
\textbf{요구 사항:} 새로운 메뉴 항목은 기존 메뉴 항목 중 하나보다 더 나은 수요를 보여야 합니다. 분석은 0.01 유의 수준에서 차이를 보여야 선택됩니다.
...
\begin{tcolorbox}[mybox, title=제품 테스트 (3/5): 데이터 수집 방법]
...
\begin{tcolorbox}[mybox, title=제품 테스트 (4/5): 결과 분석 및 p-값 결정]
\textbf{가설:}
\begin{itemize}
...
\begin{tcolorbox}[mybox, title=제품 테스트 (5/5): 최종 결론 및 경영진의 관심]
\textbf{가설:}
\begin{itemize}
    \item $H_0 : \mu_{new} - \mu_{regular} \le 0$
    \item $H_A : \mu_{new} - \mu_{regular} > 0$
...
\begin{tcolorbox}[mybox, title=수요의 차이는 얼마나 될까? (1/2) - 신뢰 구간 계산 (단측 검정 후)]
...
\begin{tcolorbox}[mybox, title=수요의 차이는 얼마나 될까? (2/2) - 최종 신뢰 구간 및 해석]
\textbf{계산된 값:}
\begin{itemize}
...
\begin{tcolorbox}[mybox, title=연습 문제: 귀무 가설 예시 (선택)]
한 교육 프로그램은 대부분의 직원의 판매 실적을 향상시킬 것을 약속합니다. 영업 부서 관리자는 직원 80명을 이 프로그램에 참여시킵니다. 무작위로 선정된 대조군보다 5\% 유의 수준에서 결과가 더 좋다고 나타나면, 모든 직원을 프로그램에 참여시킬 것입니다.
...
\begin{tcolorbox}[mybox, title=연습 문제: 풀이 - 가설 설정 (교육 프로그램)]
...
\begin{tcolorbox}[mybox, title=연습 문제: 결과 분석 (교육 프로그램)]
\textbf{가설:}
\begin{itemize}
...
\begin{tcolorbox}[mybox, title=연습 문제: 예시 표 (선택)]
...
\begin{tcolorbox}[mybox, title=연습 문제: 풀이 - 결론 (교육 프로그램)]
\textbf{가설:}
\begin{itemize}
    \item $H_0 : \mu_1 \ge \mu_2$
    \item $H_A : \mu_1 < \mu_2$
...
\begin{tcolorbox}[mybox, title=연습 문제: 풀이 - 신뢰 구간 (교육 프로그램)]
...
\begin{tcolorbox}[mybox, title=무엇을 의미하는가? (What Does It Mean?) - 교육 프로그램 신뢰 구간 해석]
...
표준 오차 & 표본 통계량(예: 평균)이 모수(예: 모평균)와 얼마나 차이 날 수 있는지 나타내는 척도 & Standard Error & 표본 변동성의 추정치 \\
...
\begin{enumerate}
    ...
    \item \textbf{오차 한계 (Margin of Error):}
    오차 한계 = $t_{\alpha/2} \times \text{표준 오차} = 1.65 \times 2.144 \approx 3.54$
    \item \textbf{90\% 신뢰 구간:}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{차이 열 생성:} Excel 데이터 옆에 'diff'라는 새 열을 만듭니다. 각 학생에 대해 '교육 후 점수 - 교육 전 점수'를 계산하여 이 열을 채웁니다. (예: `=C2-B2`)
        \item \textbf{표준 편차 계산:} 'diff' 열의 모든 값에 대해 `STDEV.S()` 함수를 사용하여 표준 편차를 계산합니다. (예: `=STDEV.S(D2:D66)`)
...
\begin{tcolorbox}[colback=myblue!5!white, colframe=myblue!75!black, title=\textbf{두 표본 비율 검정 핵심 요약}]
