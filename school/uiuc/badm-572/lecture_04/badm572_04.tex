...
\hypersetup{
    pdftitle={BADM-572: Data-Driven Decision Making - Module 04},
    pdfauthor={강의 노트},
...
\begin{enumerate}
    \item \textbf{90\% 신뢰 수준:}
        \begin{itemize}
            ...
            \item \code{=T.INV(0.95, 199)}를 입력합니다. (90\% 신뢰 구간의 오른쪽 꼬리까지의 누적 확률은 90\% + 5\% = 95\% = 0.95)
            \item \code{Enter}를 누릅니다.
        \end{itemize}
    \item \textbf{99\% 신뢰 수준:}
        \begin{itemize}
            \item 임계값을 계산할 셀을 선택합니다.
            \item \code{=T.INV(0.995, 199)}를 입력합니다. (99\% 신뢰 구간의 오른쪽 꼬리까지의 누적 확률은 99\% + 0.5\% = 99.5\% = 0.995)
            \item \code{Enter}를 누릅니다.
        \end{itemize}
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{90\% 신뢰 수준:}
        \begin{itemize}
            ...
            \item \code{= (90\% 임계값 셀) * (표준 오차 셀)}을 입력합니다.
            \item 예를 들어, 90\% 임계값이 G11셀에 있고 표준 오차가 F7셀에 있다면, \code{=G11*F7}과 같이 입력합니다.
            \item \code{Enter}를 누릅니다.
        \end{itemize}
    \item \textbf{99\% 신뢰 수준:}
        \begin{itemize}
            \item 오차 한계를 계산할 셀을 선택합니다.
            \item \code{= (99\% 임계값 셀) * (표준 오차 셀)}을 입력합니다.
            \item 예를 들어, 99\% 임계값이 H11셀에 있고 표준 오차가 F7셀에 있다면, \code{=H11*F7}과 같이 입력합니다.
            \item \code{Enter}를 누릅니다.
        \end{itemize}
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{90\% 신뢰 수준:}
        \begin{itemize}
            \item \textbf{하한:} \code{= (표본 평균 셀) - (90\% 오차 한계 셀)}
            \item \textbf{상한:} \code{= (표본 평균 셀) + (90\% 오차 한계 셀)}
        \end{itemize}
    \item \textbf{99\% 신뢰 수준:}
        \begin{itemize}
            \item \textbf{하한:} \code{= (표본 평균 셀) - (99\% 오차 한계 셀)}
            \item \textbf{상한:} \code{= (표본 평균 셀) + (99\% 오차 한계 셀)}
        \end{itemize}
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{examplebox}[title=Excel IF 함수 (표본 데이터)]
        ...
        \textbf{함수}: \code{=IF(D5>0,1,0)} (D5는 첫 번째 표본 주식의 "\excel{\% Chg}" 값)
