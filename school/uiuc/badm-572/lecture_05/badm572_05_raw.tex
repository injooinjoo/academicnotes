...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{example-sampling-distribution.png} % 이미지 파일이 없으므로 주석 처리하고 텍스트로 설명
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{example-t-distribution.png} % 이미지 파일이 없으므로 주석 처리하고 텍스트로 설명
    ...
    \begin{itemize}
        ...
        \item \textbf{p-값 계산 (Excel 함수):} Excel의 `T.DIST(x, deg_freedom, cumulative)` 함수를 사용하여 t-값보다 작거나 같은 누적 확률을 계산합니다.
        \begin{itemize}
            \item `x`: 계산된 t-값 (-3.47)
            \item `deg_freedom`: 자유도 ($n-1 = 50-1 = 49$)
            \item `cumulative`: 1 (누적 확률을 의미)
        \end{itemize}
        \item \textbf{결과:} `T.DIST(-3.47, 49, 1)` $\approx 0.00055$
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{example-p-value-two-tail.png} % 이미지 파일이 없으므로 주석 처리하고 텍스트로 설명
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\textwidth]{excel_chocolate_weights_data.png} % 이미지 파일이 없으므로 주석 처리하고 텍스트로 설명
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{평균 무게 (Mean Weight):} `E2` 셀에 `=AVERAGE(A1:A50)`을 입력합니다. 결과는 약 `294.609`가 나옵니다.
        \item \textbf{표준편차 (Standard Deviation):} `E3` 셀에 `=STDEV.S(A1:A50)`을 입력합니다. 결과는 약 `10.98013`이 나옵니다. (문제에서는 반올림하여 11로 사용)
        \item \textbf{표본 크기 (Sample Size):} `E4` 셀에 `=COUNT(A1:A50)`을 입력합니다. 결과는 `50`이 나옵니다.
        \item \textbf{유의 수준 (Significance Level):} `E5` 셀에 `0.05`를 입력합니다.
        \item \textbf{가설 평균 (Hypothesized Mean):} `E6` 셀에 `300`을 입력합니다.
    \end{itemize}

    \item \textbf{t-값 계산:}
    \begin{itemize}
        \item t-값 공식: $t = (\bar{x} - \mu_0) / (s / \sqrt{n})$
        \item `E7` 셀에 `=(E2-E6)/(E3/SQRT(E4))`를 입력합니다.
        \item 결과는 약 `-3.47174`가 나옵니다.
    \end{itemize}
    \begin{figure}[h!]
        \centering
        \includegraphics[width=0.7\textwidth]{excel_t_value_calculation.png} % 이미지 파일이 없으므로 주석 처리하고 텍스트로 설명
    ...
    \item \textbf{p-값 계산:}
    \begin{itemize}
        \item `E8` 셀에 `=T.DIST(E7, E4-1, 1)`을 입력합니다.
        \item `T.DIST` 함수는 주어진 t-값보다 작거나 같은 누적 확률을 반환합니다.
        \item 결과는 약 `0.000545`가 나옵니다. 이는 왼쪽 꼬리 부분의 확률입니다.
    \end{itemize}
    ...
    \begin{figure}[h!]
        \centering
        \includegraphics[width=0.7\textwidth]{excel_p_value_conclusion.png} % 이미지 파일이 없으므로 주석 처리하고 텍스트로 설명
...
\begin{itemize}
    ...
    \item \textbf{p-값 계산 (Excel 함수):} `T.DIST(x, deg_freedom, cumulative)`
        \begin{itemize}
            \item `x`: -1.47
            \item `deg_freedom`: $n-1 = 150-1 = 149$
            \item `cumulative`: 1
        \end{itemize}
    \item \textbf{결과:} `T.DIST(-1.47, 149, 1)` $\approx 0.0718 \approx 0.072$
\end{itemize}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.7\textwidth]{one_tail_left_p_value.png} % 이미지 파일이 없으므로 주석 처리하고 텍스트로 설명
...
\begin{itemize}
    ...
    \item \textbf{p-값 계산 (Excel 함수):} `T.DIST(x, deg_freedom, cumulative)`
        \begin{itemize}
            \item `x`: 1.85
            \item `deg_freedom`: $n-1 = 200-1 = 199$
            \item `cumulative`: 1
        \end{itemize}
    \item \textbf{결과:} `T.DIST(1.85, 199, 1)` $\approx 0.967102$
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.7\textwidth]{one_tail_right_p_value.png} % 이미지 파일이 없으므로 주석 처리하고 텍스트로 설명
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\textwidth]{excel_survey_data.png} % 이미지 파일이 없으므로 주석 처리하고 텍스트로 설명
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{평균 (Mean):} `D3` 셀에 `=AVERAGE(A1:A65)`을 입력합니다. 결과는 약 `18.03077`이 나옵니다.
        \item \textbf{표준편차 (Standard deviation):} `D4` 셀에 `=STDEV.S(A1:A65)`을 입력합니다. 결과는 약 `3.716388`이 나옵니다.
        \item \textbf{유의 수준 (Significance level):} `D5` 셀에 `0.05`를 입력합니다.
        \item \textbf{가설 평균 (hypothesized mean):} `D6` 셀에 `20`을 입력합니다.
    \end{itemize}

    \item \textbf{t-값 계산:}
    \begin{itemize}
        \item `D7` 셀에 `=(D3-D6)/(D4/SQRT(COUNT(A1:A65)))`를 입력합니다. (COUNT 함수로 n을 직접 계산)
        \item 결과는 약 `-4.27201`이 나옵니다.
    \end{itemize}
    \begin{figure}[h!]
        \centering
        \includegraphics[width=0.7\textwidth]{excel_survey_t_value.png} % 이미지 파일이 없으므로 주석 처리하고 텍스트로 설명
    ...
    \item \textbf{p-값 계산:}
    \begin{itemize}
        \item `D8` 셀에 `=T.DIST(D7, COUNT(A1:A65)-1, 1)`을 입력합니다.
        \item `T.DIST` 함수는 주어진 t-값보다 작거나 같은 누적 확률을 반환하며, 이는 좌측 단측 검정의 p-값에 해당합니다.
        \item 결과는 약 `3.27938E-05` (즉, 0.0000327938)가 나옵니다.
    \end{itemize}
    ...
    \begin{figure}[h!]
        \centering
        \includegraphics[width=0.7\textwidth]{excel_survey_p_value_conclusion.png} % 이미지 파일이 없으므로 주석 처리하고 텍스트로 설명
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Excel의 `AVERAGE`, `STDEV.S`, `COUNT`, `SQRT`, `T.DIST` 함수를 사용하여 p-값을 계산할 수 있는가?
...
\begin{tcolorbox}[colback=summarycolor, colframe=blue!75!black, title=\textbf{모듈 1 핵심 요약: 가설 검정의 기초}]
...
\begin{itemize}
    ...
    \item \textbf{Excel 함수:}
        \begin{itemize}
            \item `T.DIST(t, df, 1)`: t-값보다 작거나 같은 누적 확률 (왼쪽 꼬리).
            \item `1 - T.DIST(t, df, 1)`: t-값보다 크거나 같은 누적 확률 (오른쪽 꼬리).
        \end{itemize}
    \item \textbf{결론 규칙:}
        \begin{itemize}
...
\textbf{5. Excel 실습:}
\begin{itemize}
    \item `AVERAGE`, `STDEV.S`, `COUNT`, `SQRT`로 기본 통계량 계산.
    \item `T.DIST`로 p-값 계산 후 $\alpha$와 비교하여 의사결정.
\end{itemize}
\end{tcolorbox}
...
\begin{enumerate}
    ...
    $$t = \frac{\bar{x} - \mu_0}{s/\sqrt{n}}$$
    Excel에서는 각 값이 입력된 셀을 참조하여 계산합니다. 예를 들어, $\bar{x}$가 E3, $\mu_0$가 E8, $s$가 E4, $n$이 E5 셀에 있다면, `=(E3-E8)/(E4/SQRT(E5))`와 같이 입력합니다.
    \item \textbf{p-값 계산}: 우측 꼬리 검정의 p-값은 t-분포에서 계산된 t-값보다 큰 영역의 확률입니다. Excel의 \texttt{T.DIST} 함수는 왼쪽 꼬리 누적 확률을 반환하므로, 1에서 이 값을 빼야 합니다.
    \begin{itemize}
        \item 자유도 (df): $n-1$
        \item Excel 수식: `1 - T.DIST(t-값, 자유도, 1)`
        \item 예시: `1 - T.DIST(E11, E5-1, 1)` (E11에 t-값이, E5에 n이 있다고 가정)
    \end{itemize}
    \item \textbf{결론 도출}:
    \begin{itemize}
...
\begin{enumerate}
    ...
    $$z = \frac{\hat{p} - p_0}{\sqrt{\frac{p_0(1-p_0)}{n}}}$$
    Excel에서는 각 값이 입력된 셀을 참조하여 계산합니다. 예를 들어, $\hat{p}$가 G3, $p_0$가 G4, $n$이 G5 셀에 있다면, `=(G3-G4)/SQRT(G4*(1-G4)/G5)`와 같이 입력합니다.
    \item \textbf{p-값 계산}: 좌측 꼬리 검정의 p-값은 z-분포에서 계산된 z-값보다 작거나 같은 영역의 확률입니다. Excel의 \texttt{NORM.S.DIST} 함수는 기본적으로 왼쪽 꼬리 누적 확률을 반환합니다.
    \begin{itemize}
        \item Excel 수식: `NORM.S.DIST(z-값, 1)`
        \item 예시: `NORM.S.DIST(G7, 1)` (G7에 z-값이 있다고 가정)
    \end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Excel 수식: `1 - NORM.S.DIST(z-값, 1)`
        \item 예시: `1 - NORM.S.DIST(I7, 1)` (I7에 z-값이 있다고 가정)
    \end{itemize}
    \item \textbf{결론 도출}: p-값 < $\alpha$ 이면 귀무가설을 기각하고, 대립가설이 지지됩니다.
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item z-값이 양수일 경우 (오른쪽 꼬리): `1 - NORM.S.DIST(z-값, 1)`
            \item z-값이 음수일 경우 (왼쪽 꼬리): `NORM.S.DIST(z-값, 1)`
        \end{itemize}
        \item 이 한쪽 꼬리 p-값에 2를 곱하여 양측 p-값을 구합니다.
        \item 예시: `(1 - NORM.S.DIST(ABS(I7), 1)) * 2` (I7에 z-값이 있다고 가정, ABS는 절댓값 함수)
    \end{itemize}
...
\begin{itemize}
    \item \texttt{T.DIST(x, deg\_freedom, cumulative)}: x보다 작거나 같은 값의 누적 확률(왼쪽 꼬리)을 반환합니다. 우측 꼬리 p-값을 구하려면 `1 - T.DIST(...)`를 사용해야 합니다.
    \item \texttt{NORM.S.DIST(z, cumulative)}: z보다 작거나 같은 값의 누적 확률(왼쪽 꼬리)을 반환합니다. 우측 꼬리 p-값을 구하려면 `1 - NORM.S.DIST(...)`를 사용해야 합니다.
...
\begin{tcolorbox}[colback=myblue!5!white, colframe=myblue!75!black, title=\textbf{가설 검정 핵심 요약}]
\textbf{목표}: 모집단에 대한 주장을 표본 데이터로 평가

\textbf{4단계 절차}:
\begin{enumerate}
    \item \textbf{가설 설정}: $H_0$ (등호 포함, 현재 믿음), $H_A$ (우리가 증명할 것)
    \item \textbf{유의수준 ($\alpha$)}: 제1종 오류 허용치 (예: 0.05)
    \item \textbf{p-값 계산}: 귀무가설 하에서 관측 데이터의 극단성 확률
    \item \textbf{결론}: p-값 < $\alpha$ $\rightarrow$ $H_0$ 기각 (대립가설 지지)
\end{enumerate}
\end{tcolorbox}

\begin{tcolorbox}[colback=mygreen!5!white, colframe=mygreen!75!black, title=\textbf{평균 검정 (t-검정)}]
\textbf{언제}: 모집단 평균($\mu$)에 대한 주장 검정
\textbf{검정 통계량}: $t = \frac{\bar{x} - \mu_0}{s/\sqrt{n}}$
\textbf{Excel p-값}:
\begin{itemize}
    \item \textbf{우측 꼬리 ($H_A: \mu > \mu_0$)}: \texttt{1 - T.DIST(t, df, 1)}
\end{itemize}
\textbf{주의}: \texttt{T.DIST}는 왼쪽 꼬리 누적 확률 반환
\end{tcolorbox}

\begin{tcolorbox}[colback=mygreen!5!white, colframe=mygreen!75!black, title=\textbf{비율 검정 (z-검정)}]
\textbf{언제}: 모집단 비율($p$)에 대한 주장 검정
\textbf{정규 근사 조건}: $n \times p_0 \ge 5$ \textbf{AND} $n \times (1 - p_0) \ge 5$
\textbf{검정 통계량}: $z = \frac{\hat{p} - p_0}{\sqrt{\frac{p_0(1-p_0)}{n}}}$
\textbf{Excel p-값}:
\begin{itemize}
    \item \textbf{좌측 꼬리 ($H_A: p < p_0$)}: \texttt{NORM.S.DIST(z, 1)}
    \item \textbf{우측 꼬리 ($H_A: p > p_0$)}: \texttt{1 - NORM.S.DIST(z, 1)}
    \item \textbf{양측 꼬리 ($H_A: p \ne p_0$)}: \texttt{(1 - NORM.S.DIST(ABS(z), 1)) * 2}
\end{itemize}
\textbf{주의}: \texttt{NORM.S.DIST}는 왼쪽 꼬리 누적 확률 반환
\end{tcolorbox}

\begin{tcolorbox}[colback=myred!5!white, colframe=myred!75!black, title=\textbf{중요한 오해}]
\begin{itemize}
    \item \textbf{통계적 유의성 $\ne$ 실질적 중요성}: p-값이 작다고 항상 중요한 발견은 아님.
    \item \textbf{$H_0$ 기각 실패 $\ne$ $H_0$ 참}: 증거가 부족할 뿐, 참이라고 단정할 수 없음.
\end{itemize}
\end{tcolorbox}
