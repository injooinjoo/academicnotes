...
단순 선형 회귀(Simple Linear Regression)는 하나의 독립 변수(원인)로 하나의 종속 변수(결과)를 예측하는 데 사용됩니다. 하지만 현실의 많은 문제들은 여러 원인이 복합적으로 작용하여 결과를 만들어냅니다. 이러한 복잡한 상황을 분석하고 예측하기 위해 **다중 선형 회귀(Multiple Linear Regression)**가 필요합니다. 다중 선형 회귀는 여러 독립 변수를 동시에 고려하여 종속 변수를 예측함으로써, 단순 선형 회귀보다 훨씬 더 정교하고 현실적인 분석을 가능하게 합니다.
\end{tcolorbox}
...
다중 선형 회귀도 단순 선형 회귀와 마찬가지로 **최소 제곱법(Least Squares Method)**을 사용하여 회귀 방정식을 찾습니다. 최소 제곱법은 실제 값과 예측 값 사이의 오차(잔차) 제곱합을 최소화하는 회귀선을 찾는 방법입니다.
...
\begin{enumerate}
    ...
    \item \textbf{입력 Y 범위 설정:} '회귀 분석' 대화 상자에서 [입력 Y 범위(Input Y Range)]에 종속 변수(Sales)가 있는 열 전체를 선택합니다. (예: `$A$1:$A$51`)
    \item \textbf{입력 X 범위 설정:} [입력 X 범위(Input X Range)]에 \textbf{모든 독립 변수(Promotional Budget, Time, Market Potential, Market Change, Customer Rating)가 있는 연속된 열 전체}를 선택합니다. (예: `$B$1:$F$51`)
...
\begin{enumerate}
    ...
    \end{itemize}
    \item \textbf{예측 계산 (SUMPRODUCT 함수 사용):} Excel의 `SUMPRODUCT` 함수를 사용하여 예측 값을 계산합니다. 이 함수는 두 배열의 해당 요소들을 곱한 후 그 합계를 반환합니다.
...
=Intercept_Coefficient + SUMPRODUCT(Coefficients_Range, X_Values_Range)
    ...
    `=B28 + SUMPRODUCT(B29:B32, C29:C32)`
    \item \textbf{계수 범위 고정:} 만약 여러 시나리오에 대해 예측을 반복하고 싶다면, `SUMPRODUCT` 함수 내에서 계수 범위(예: `B29:B32`)를 절대 참조(예: `$B$29:$B$32`)로 고정합니다. (셀을 선택하고 `F4` 키를 누르면 됩니다.) 이렇게 하면 X 값만 변경하면서 쉽게 예측 값을 얻을 수 있습니다.
...
이 문서는 회귀 모델에 질적(범주형) 데이터를 포함하는 방법을 다룹니다.
특히, 두 가지 범주를 가진 질적 변수(예: 성별)와 여러 범주를 가진 질적 변수(예: 전공)를
더미 변수(Dummy Variable)를 사용하여 회귀 분석에 통합하는 과정을 상세히 설명합니다.
더미 변수의 생성 원리, 다중 공선성(Collinearity) 회피 방법,
그리고 Excel을 활용한 실제 분석 및 결과 해석에 중점을 둡니다.
이를 통해 비전공자도 회귀 모델에서 질적 변수의 중요성과 활용법을 완벽히 이해할 수 있도록 돕습니다.

\newpage
\section*{용어 정리}
\addcontentsline{toc}{section}{용어 정리}
회귀 분석에서 질적 데이터를 효과적으로 사용하기 위해 알아야 할 핵심 용어들을 정리했습니다.
...
\textbf{용어} & \textbf{쉬운 설명} & \textbf{원어} & \textbf{비고} \\
\midrule
질적 데이터 & 숫자보다는 특성이나 범주를 나타내는 데이터 & Qualitative Data & 성별, 전공, 우편번호 등 \\
범주형 변수 & 몇 가지 정해진 범주 중 하나를 가지는 변수 & Categorical Variable & 질적 데이터의 한 종류 \\
더미 변수 & 범주형 변수를 숫자로 표현하기 위해 0 또는 1로 코딩한 변수 & Dummy Variable & 회귀 모델에 질적 데이터를 넣는 방법 \\
지표 변수 & 더미 변수와 동일한 의미로 사용되는 용어 & Indicator Variable & Dummy Variable과 상호 교환 가능 \\
다중 공선성 & 독립 변수들끼리 서로 강하게 상관되어 있는 현상 & Collinearity & 회귀 모델의 신뢰성을 해칠 수 있음 \\
절편 & 모든 독립 변수가 0일 때 종속 변수의 예측값 & Intercept & 기준 범주(0으로 코딩된)의 기본값으로 해석될 수 있음 \\
회귀 계수 & 독립 변수가 한 단위 변할 때 종속 변수에 미치는 영향의 크기 & Regression Coefficient & 더미 변수의 경우, 기준 범주 대비 차이를 나타냄 \\
\bottomrule
\end{tabular}
\end{adjustbox}
...
\addcontentsline{toc}{section}{핵심 개념 및 원리: 질적 데이터와 더미 변수}
...
\addcontentsline{toc}{section}{절차/방법: Excel에서 더미 변수 사용하기 (두 가지 범주)}
Excel을 사용하여 범주형 변수를 더미 변수로 코딩하고 회귀 분석을 실행하는 구체적인 단계를 설명합니다.
...
\addcontentsline{toc}{section}{핵심 개념 및 원리: 여러 범주를 가진 질적 데이터 처리}
성별처럼 두 가지 범주만 있는 경우가 아니라, 전공처럼 여러 가지 범주를 가진 질적 변수를 회귀 모델에 포함하는 방법을 살펴봅니다.
...
\begin{tcolorbox}[title=더미 변수 생성 규칙 (여러 범주)]
...
\newpage
\section*{개요}
이 문서는 여러 범주를 가진 질적 데이터를 회귀 분석에 활용하기 위한 더미 변수(Dummy Variable) 생성 및 적용 방법을 설명합니다. 특히 엑셀을 사용하여 다중 범주 더미 변수를 포함한 회귀 모델을 구축하고, 그 결과를 해석하며, 새로운 데이터에 대한 예측을 수행하는 과정을 다룹니다. N개의 범주를 가진 질적 변수를 모델링할 때 N-1개의 더미 변수를 사용하는 'N-1 규칙'의 중요성과 실제 적용 사례를 통해 비전공자도 쉽게 이해할 수 있도록 구성되었습니다.
...
\textbf{용어} & \textbf{쉬운 설명} & \textbf{원어} & \textbf{비고} \\
\midrule
더미 변수 & 질적 데이터를 수량화하여 회귀 모델에 사용할 수 있도록 0 또는 1로 코딩한 변수. & Dummy Variable & \\
범주형 데이터 & 성별, 전공, 지역 등 몇 가지 정해진 범주로 나눌 수 있는 데이터. & Categorical Data & 질적 데이터라고도 함. \\
기준 범주 & 더미 변수를 만들 때 0으로 코딩되어 다른 모든 범주와 비교되는 기준이 되는 범주. & Reference Category & N-1 규칙에 따라 하나는 제외됨. \\
회귀 분석 & 하나 이상의 독립 변수가 종속 변수에 미치는 영향을 분석하고 예측하는 통계 기법. & Regression Analysis & \\
R-제곱 조정 & 모델이 종속 변수의 분산을 얼마나 잘 설명하는지 나타내는 지표로, 독립 변수 수에 따라 조정됨. & Adjusted R-Squared & 모델의 설명력을 평가. \\
p-값 & 관찰된 결과가 우연히 발생할 확률. 일반적으로 0.05보다 작으면 통계적으로 유의미하다고 판단. & p-value & 변수의 유의성 판단 기준. \\
계수 & 독립 변수가 한 단위 변할 때 종속 변수가 변하는 정도를 나타내는 값. & Coefficient & 더미 변수의 경우 기준 범주 대비 효과. \\
예측 & 구축된 회귀 모델을 사용하여 새로운 독립 변수 값에 대한 종속 변수 값을 추정하는 과정. & Prediction & \\
\bottomrule
\end{tabular}
\end{adjustbox}
...
\begin{example}{예시: 전공 데이터를 더미 변수로 변환}
...
\begin{enumerate}
    ...
    \item \textbf{입력 범위 설정}:
    \begin{itemize}
        \item \textbf{Input Y Range (종속 변수)}: 예측하고자 하는 '시작 연봉(Starting Salary)' 열 전체를 선택합니다. (예: `$A$1:$A$101`)
        \item \textbf{Input X Range (독립 변수)}: 'GPA', 'Engineering', 'Business', 'Liberal Arts' 열 전체를 선택합니다. (예: `$B$1:$E$101`)
    \end{itemize}
    \item \textbf{옵션 설정}:
    \begin{itemize}
...
엑셀에서는 `SUMPRODUCT` 함수를 사용하여 이 계산을 효율적으로 수행할 수 있습니다.

\begin{lstlisting}[caption={엑셀 SUMPRODUCT 함수를 이용한 예측 공식}, label={lst:sumproduct_formula}]
=$B$24 + SUMPRODUCT($B$25:$B$28, C25:C28)
\end{lstlisting}
여기서 `$B$24`는 절편 값, `$B$25:$B$28`은 GPA 및 더미 변수들의 계수 범위, `C25:C28`은 예측하고자 하는 학생의 GPA 및 더미 변수 값 범위입니다.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item 엑셀의 `SUMPRODUCT` 함수를 활용하여 예측을 자동화할 수 있는가?
...
\begin{tcolorbox}[summary={핵심 요약: 다중 범주 더미 변수}]
...
\begin{itemize}
    ...
    \item 엑셀 `SUMPRODUCT` 함수로 쉽게 계산 가능.
\end{itemize}
\end{tcolorbox}
