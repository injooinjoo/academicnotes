...
\begin{tcolorbox}[mybox, title=회귀 분석의 주요 목표 (Objectives of Regression Analysis)]
...
\begin{tcolorbox}[mybox, title=회귀 (Regression)]
...
\begin{tcolorbox}[mybox, title=단순 선형 회귀 (Simple Linear Regression)]
...
\begin{center}
\includegraphics[width=0.8\textwidth]{example-spurious-correlation.png} % 실제 이미지는 없으므로 텍스트로 대체
\end{center}
...
\begin{tcolorbox}[mybox, title=산점도 (Scatter Plots)]
산점도는 우리가 식별한 변수들 사이에 어떤 특별한 관계가 있는지 시각적으로 감지하는 훌륭한 도구입니다.

\begin{center}
\includegraphics[width=0.8\textwidth]{example-scatter-plot-income-education.png} % 실제 이미지는 없으므로 텍스트로 대체
\end{center}
...
\begin{tcolorbox}[mybox, title=가능한 선형 관계 (Possible Linear Relationships)]
두 변수를 플로팅하여 다음과 같은 관계를 감지할 수 있습니다.
\begin{center}
\includegraphics[width=0.9\textwidth]{example-linear-relationships.png} % 실제 이미지는 없으므로 텍스트로 대체
\end{center}
...
\begin{center}
\includegraphics[width=0.8\textwidth]{example-romney-vote-income.png} % 실제 이미지는 없으므로 텍스트로 대체
\end{center}
...
\begin{tcolorbox}[mybox, title=산점도 분석 (Looking at Scatter Plots)]
...
\begin{enumerate}[label=\arabic*.]
    ...
    \begin{center}
    \includegraphics[width=0.8\textwidth]{example-romney-vote-income-outlier.png} % 실제 이미지는 없으므로 텍스트로 대체
    \end{center}
...
\begin{tcolorbox}[mybox, title=회귀 방정식 (Regression Equation)]
\begin{enumerate}[label=\arabic*.]
...
\begin{center}
\includegraphics[width=0.6\textwidth]{example-regression-equation-components.png} % 실제 이미지는 없으므로 텍스트로 대체
\end{center}
...
\begin{center}
\includegraphics[width=0.6\textwidth]{example-regression-error-single.png} % 실제 이미지는 없으므로 텍스트로 대체
\end{center}
...
\begin{center}
\includegraphics[width=0.6\textwidth]{example-regression-errors-all.png} % 실제 이미지는 없으므로 텍스트로 대체
\end{center}
...
\begin{tcolorbox}[mybox, title=최적 적합선 (Line of Best Fit)]
...
\begin{tcolorbox}[mybox, title=최소 제곱 점 추정치 (Least Square Point Estimates)]
예측 방정식은 다음과 같습니다.
\begin{equation*}
\hat{y} = b_0 + b_1x
\end{equation*}
...
\begin{center}
\includegraphics[width=0.8\textwidth]{example-regression-line-sales.png} % 실제 이미지는 없으므로 텍스트로 대체
\end{center}
...
\begin{tcolorbox}[mybox, title=Excel 출력 (Excel Output)]
...
\begin{enumerate}[label=\arabic*.]
    ...
    \begin{itemize}
        ...
        \item 'xy' 열을 추가하고 각 행에서 X와 Y 값을 곱합니다 (예: `C2*D2`).
        \item 'x$^2$' 열을 추가하고 각 행에서 X 값을 제곱합니다 (예: `C2^2`).
    \end{itemize}

    \item \textbf{합계 계산}:
    \begin{itemize}
        \item 각 열 (x, y, xy, x$^2$)의 합계를 계산합니다. Excel의 `SUM` 함수를 사용합니다.
        \item 예를 들어, X 열의 합계는 `SUM(C2:C9)`와 같이 계산됩니다.
    \end{itemize}
    \begin{adjustbox}{max width=\textwidth}
    \begin{tabular}{ccccc}
    \toprule
    ...
    \end{equation*}
    Excel에서: `=(G21-((E21*F21)/8))` (여기서 G21은 xy 합계, E21은 x 합계, F21은 y 합계, 8은 n 값)
    결과: `56239.48375`
    ...
    \end{equation*}
    Excel에서: `=(H21-((E21^2)/8))` (여기서 H21은 x$^2$ 합계, E21은 x 합계, 8은 n 값)
    결과: `839.78875` (원문 슬라이드에서는 80.078로 표기되었으나, 계산 결과는 다름. 여기서는 계산된 값을 따름)

    \item \textbf{기울기 ($b_1$) 계산}:
    \begin{equation*}
    b_1 = \frac{SS_{xy}}{SS_{xx}}
    \end{equation*}
    Excel에서: `=(SSxy_cell)/(SSxx_cell)`
    결과: `56239.48375 / 839.78875 = 66.968` (원문 슬라이드에서는 66.94549023로 표기됨. 여기서는 계산된 값을 따름)

    \item \textbf{절편 ($b_0$) 계산}:
    \begin{equation*}
    b_0 = \bar{y} - b_1\bar{x}
    \end{equation*}
    Excel에서: `=AVERAGE(Y_values_range)-(b1_cell*AVERAGE(X_values_range))`
    결과: `1714.3875 - (66.968 * 29.8375) = -283.0985647` (원문 슬라이드와 동일)
...
\begin{enumerate}[label=\arabic*.]
    ...
    \item \textbf{입력 Y 범위 (Input Y Range)}: 종속 변수(Annual sales)가 있는 열을 선택합니다. (예: `$B$1:$B$9`)
    \item \textbf{입력 X 범위 (Input X Range)}: 독립 변수(Population)가 있는 열을 선택합니다. (예: `$C$1:$C$9`)
...
\begin{center}
\includegraphics[width=0.8\textwidth]{example-temp-test-score-scatter.png} % 실제 이미지는 없으므로 텍스트로 대체
\end{center}
...
\begin{center}
\includegraphics[width=0.8\textwidth]{example-temp-test-score-line.png} % 실제 이미지는 없으므로 텍스트로 대체
\end{center}
...
\begin{tcolorbox}[mybox, title=모델의 유의성 평가 (Assessing Significance of Model)]
...
\textbf{$R^2$ 해석하기}:
\begin{center}
\includegraphics[width=0.6\textwidth]{example-r-squared-scale.png} % 실제 이미지는 없으므로 텍스트로 대체
\end{center}
...
\begin{tcolorbox}[mybox, title=단순 상관 계수 (Simple Correlation Coefficient, $r$)]
...
\end{itemize}
\begin{center}
\includegraphics[width=0.6\textwidth]{example-correlation-scale.png} % 실제 이미지는 없으므로 텍스트로 대체
\end{center}
...
\begin{enumerate}[label=\arabic*.]
    ...
    \item '회귀(Regression)'를 선택하고 '확인(OK)'을 클릭합니다.
    \item \textbf{입력 Y 범위 (Input Y Range)}: 'Price' 열을 선택합니다. (예: `$B$1:$B$2326`)
    \item \textbf{입력 X 범위 (Input X Range)}: 'Square Footage' 열을 선택합니다. (예: `$C$1:$C$2326`)
...
\begin{itemize}
    ...
    \begin{center}
    \includegraphics[width=0.7\textwidth]{example-zillow-regression-line.png} % 실제 이미지는 없으므로 텍스트로 대체
    \end{center}
...
\begin{titlepage}
    \centering
    \vspace*{1cm}
    {\Huge\bfseries 추론 및 예측 통계 모듈 3}\\[1cm]
    {\Large\bfseries 모델 활용 및 가정}\\[0.5cm]
    {\Large BADM-572: 데이터 기반 의사결정}\\[2cm]
    {\Large UIUC Faculty}\\[0.5cm]
    {\large 작성일: \today}\\[3cm]
    \vfill
\end{titlepage}
...
\section*{개요}
이 문서는 '추론 및 예측 통계 모듈 3'의 후반부 내용을 다루며, 단순 선형 회귀 모델의 실제 활용법, Excel을 이용한 분석 절차, 그리고 모델의 기본 가정과 한계점을 심층적으로 설명합니다. 회귀 분석을 통해 변수 간의 관계를 이해하고 미래를 예측하는 방법을 배우며, 특히 독립 변수와 종속 변수를 올바르게 설정하는 것의 중요성을 강조합니다. 또한, 회귀 모델의 결과(R-제곱, p-값, 계수)를 해석하는 방법과 모델의 신뢰성을 검증하기 위한 잔차 분석(residual analysis)의 중요성을 다룹니다. 마지막으로, 모델의 예측 범위 제한(외삽 금지)과 상관관계가 인과관계를 의미하지 않음을 명확히 하여, 데이터 기반 의사결정 시 발생할 수 있는 오해를 방지합니다.

\newpage
\section*{용어 정리}
회귀 분석을 이해하는 데 필요한 주요 용어들을 정리했습니다. 각 용어의 의미를 명확히 파악하면 복잡한 개념도 쉽게 이해할 수 있습니다.
...
\textbf{용어} & \textbf{쉬운 설명} & \textbf{원어} & \textbf{비고} \\
\midrule
\textbf{독립 변수} & 결과에 영향을 미친다고 가정하는 변수 (원인) & Independent Variable (x) & X축에 배치 \\
\textbf{종속 변수} & 독립 변수의 영향을 받는다고 가정하는 변수 (결과) & Dependent Variable (y) & Y축에 배치 \\
\textbf{산점도} & 두 변수 간의 관계를 점으로 나타낸 그래프 & Scatter Plot & 선형성, 상관관계 시각화 \\
\textbf{양의 상관관계} & 한 변수가 증가할 때 다른 변수도 증가하는 관계 & Positive Correlation & 우상향하는 경향 \\
\textbf{음의 상관관계} & 한 변수가 증가할 때 다른 변수는 감소하는 관계 & Negative Correlation & 우하향하는 경향 \\
\textbf{회귀 방정식} & 독립 변수와 종속 변수 간의 관계를 나타내는 수학식 & Regression Equation & 예측 모델 \\
\textbf{절편} & 독립 변수가 0일 때 종속 변수의 예측값 & Intercept ($b_0$) & 회귀선이 Y축과 만나는 점 \\
\textbf{기울기 (계수)} & 독립 변수가 1단위 변할 때 종속 변수의 예측 변화량 & Coefficient ($b_1$) & 회귀선의 경사도 \\
\textbf{R-제곱} & 종속 변수 변동의 몇 퍼센트가 독립 변수로 설명되는지 & R-Squared ($R^2$) & 모델 설명력, 0~1 사이 값 \\
\textbf{다중 R} & 두 변수 간의 선형 상관관계 강도와 방향 & Multiple R & 상관계수 ($r$), -1~1 사이 값 \\
\textbf{p-값} & 귀무 가설이 참일 확률. 작을수록 통계적으로 유의미 & p-value & 관계의 유의미성 판단 \\
\textbf{귀무 가설} & 변수들 사이에 아무런 관계가 없다는 가설 & Null Hypothesis ($H_0$) & 기각될수록 관계가 있다고 봄 \\
\textbf{잔차} & 실제 값과 모델의 예측 값 사이의 차이 (오차) & Residual (e) & 모델 가정 검증에 사용 \\
\textbf{정규성 가정} & 잔차가 정규 분포를 따른다는 가정 & Normality Assumption & 잔차 히스토그램/정규 확률 플롯으로 확인 \\
\textbf{등분산성 가정} & 잔차의 분산이 독립 변수 값에 관계없이 일정하다는 가정 & Constant Variance (Homoscedasticity) & 잔차 플롯에서 패턴 없이 고르게 분포 \\
\textbf{선형성 가정} & 독립 변수와 종속 변수 간의 관계가 선형이라는 가정 & Linearity Assumption & 산점도로 확인 \\
\textbf{독립성 가정} & 각 잔차가 서로 독립적이라는 가정 & Independence Assumption & 시계열 데이터에서 위반되기 쉬움 \\
\textbf{자기상관} & 시간 순서에 따라 오차 항들이 서로 상관되어 있는 현상 & Autocorrelation & 독립성 가정 위반의 한 형태 \\
\textbf{외삽} & 모델을 학습시킨 데이터 범위를 벗어나 예측하는 행위 & Extrapolation & 예측의 신뢰도를 떨어뜨림 \\
\textbf{인과관계} & 한 변수가 다른 변수의 직접적인 원인이 되는 관계 & Causation & 상관관계와 혼동 금지 \\
\textbf{상관관계} & 두 변수가 함께 변화하는 경향 & Correlation & 인과관계와 다름 \\
\bottomrule
\end{tabular}
\end{adjustbox}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{example-positive-correlation-scatter-plot.png} % Placeholder image
    \caption{경력 연수와 급여의 산점도 (양의 상관관계)}
    \label{fig:positive_correlation_scatter}
\end{figure}
\begin{tcolorbox}[colback=myblue!10!white,colframe=myblue!50!black,title={산점도 분석: 양의 상관관계}]
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{example-wrong-scatter-plot.png} % Placeholder image
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{example-negative-correlation-scatter-plot.png} % Placeholder image
    \caption{가동 중단 시간과 생산성의 산점도 (음의 상관관계)}
    \label{fig:negative_correlation_scatter}
\end{figure}
\begin{tcolorbox}[colback=myblue!10!white,colframe=myblue!50!black,title={산점도 분석: 음의 상관관계}]
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{example-gpa-salary-scatter-plot.png} % Placeholder image
    \caption{GPA와 시작 급여의 산점도}
    \label{fig:gpa_salary_scatter}
\end{figure}
\begin{tcolorbox}[colback=myblue!10!white,colframe=myblue!50!black,title={산점도 분석: GPA와 시작 급여}]
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{excel-downtime-data.png} % Placeholder image
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{excel-downtime-scatter-plot-formatted.png} % Placeholder image
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{excel-regression-dialog.png} % Placeholder image
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{excel-downtime-output-raw.png} % Placeholder image
...
\begin{tcolorbox}[colback=myblue!10!white,colframe=myblue!50!black,title={회귀 모델의 주요 가정}]
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{residual-normal-plot.png} % Placeholder image
...
\begin{itemize}
    ...
    \begin{figure}[h!]
        \centering
        \includegraphics[width=0.8\textwidth]{residual-plots-good-bad.png} % Placeholder image
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{excel-gpa-salary-data-scatter.png} % Placeholder image
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{excel-gpa-salary-regression-dialog.png} % Placeholder image
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{excel-gpa-salary-plots.png} % Placeholder image
...
\begin{itemize}
    ...
    \begin{figure}[h!]
        \centering
        \includegraphics[width=0.6\textwidth]{excel-gpa-salary-normal-plot.png} % Placeholder image
        \caption{GPA-시작 급여 잔차의 정규 확률 플롯}
        \label{fig:gpa_salary_normal_plot}
    \end{figure}
    \begin{tcolorbox}[colback=myblue!10!white,colframe=myblue!50!black,title={정규성 가정 확인}]
    ...
    \begin{figure}[h!]
        \centering
        \includegraphics[width=0.6\textwidth]{excel-gpa-salary-residual-plot.png} % Placeholder image
        \caption{GPA-시작 급여 잔차 플롯}
        \label{fig:gpa_salary_residual_plot}
    \end{figure}
    \begin{tcolorbox}[colback=myblue!10!white,colframe=myblue!50!black,title={평균 0 및 등분산성 가정 확인}]
    \begin{itemize}
        ...
        \begin{figure}[h!]
            \centering
            \includegraphics[width=0.6\textwidth]{excel-residual-fanning-out.png} % Placeholder image
            \caption{등분산성 위반 예시: 잔차가 퍼지는 경우}
            \label{fig:residual_fanning_out}
        \end{figure}
        \begin{figure}[h!]
            \centering
            \includegraphics[width=0.6\textwidth]{excel-residual-fanning-in.png} % Placeholder image
    ...
    \begin{figure}[h!]
        \centering
        \includegraphics[width=0.6\textwidth]{excel-gpa-salary-line-fit-plot.png} % Placeholder image
        \caption{GPA-시작 급여 선형 적합 플롯}
        \label{fig:gpa_salary_line_fit_plot}
    \end{figure}
    \begin{tcolorbox}[colback=myblue!10!white,colframe=myblue!50!black,title={선형성 가정 확인}]
...
\begin{tcolorbox}[colback=myblue!10!white,colframe=myblue!50!black,title={자주 묻는 질문}]
...
\begin{tcolorbox}[colback=myblue!10!white,colframe=myblue!50!black,title={모델 활용 및 가정: 핵심 요약}]
\textbf{1. 모델 활용의 기본}
\begin{itemize}
